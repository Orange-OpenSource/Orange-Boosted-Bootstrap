---
title: Docs reference
description: Examples of OUDS Web’s documentation-specific components and styles.
aliases: "/docsref/"
toc: true
robots: noindex,follow
---

## Callouts

<Callout>
  Default callout
</Callout>

<Callout type="warning">
  Warning callout
</Callout>

<Callout type="negative">
  Danger callout
</Callout>

## Code example

<Code buttonLabel="test class" lang="scss" code={`.test {
    --color: blue;
  }`} />

<Example showMarkup={false} code={`The <abbr title="HyperText Markup Language">HTML</abbr> abbreviation element.`} />

<Example buttonLabel="this is a test" code={`<div class="test">This is a test.</div>`} />

<ScssDocs name="variable-gradient" file="scss/_variables.scss" />

<JsDocs name="live-toast" file="../../apps/site/src/assets/partials/snippets.js" />
