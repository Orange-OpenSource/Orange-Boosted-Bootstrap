...
- <span class="tag tag-small rounded-none text-bg-status-negative-emphasized">Breaking</span> Responsive breakpoints have changed:
  ...
  - All gutter utilities have been changed. `.g{-breakpoint}-{value}`, `.gx{-breakpoint}-{value}` and `.gy{-breakpoint}-{value}` which value is inside `0|1|2|3|4|5`. Gutter utilities values now use `none|3xs|2xs|xs|sm|md|lg|xl|2xl|3xl|4xl|5xl`. Proportional equivalence between `0` → `none`, `1` → `2xs`, `2` → `sm`, `3` → `lg`, `4` → `2xl`, `5` → `4xl`.
...
- <span class="tag tag-small rounded-none text-bg-status-positive-emphasized">New</span> `.badge-xs`, `.badge-sm`, `.badge-lg` have been added to size the badge.
...
- <span class="tag tag-small rounded-none text-bg-status-positive-emphasized">New</span> Shadows utilities:
