...
- <span class="tag tag-small rounded-none tag-negative">Breaking</span> OUDS Web now uses [new functional icons]([[docsref:/extend/icons#functional-icons]]). A new [bicolor accessible warning icon]([[docsref:/extend/icons#accessible-warning-icon]]) has been introduced.
...
```
See [our new Select Input page]([[docsref:/forms/select-input]]) for more information.
</div>
