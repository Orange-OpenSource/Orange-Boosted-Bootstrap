...
-<span class="badge text-bg-status-negative-emphasized">Breaking</span> Link CSS variables have been changed. All the variables containing `arrow` have been renamed the same with `chevron` instead.

### Utilities

#### Colors

-<span class="badge text-bg-status-warning-emphasized">Warning</span> Dark mode values of our `.text-status-info`, `.text-status-positive`, `.text-status-warning` and `.text-status-danger` have been tweaked. It shouldn’t bother you too much since it's handled on OUDS Web side.
