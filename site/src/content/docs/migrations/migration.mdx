...
- <span class="tag tag-small rounded-none text-bg-status-warning-emphasized">Warning</span> `.rounded-small` and `.rounded-medium` have been updated to respectively 4px and 8px. All components have been updated accordingly. It shouldn’t bother you too much since it’s handled on OUDS Web side.

#### Colors

- <span class="tag tag-small rounded-none text-bg-status-warning-emphasized">Warning</span> Dark mode values of our `.text-status-info`, `.text-status-positive`, `.text-status-warning` and `.text-status-danger` have been tweaked. It shouldn’t bother you too much since it’s handled on OUDS Web side.
