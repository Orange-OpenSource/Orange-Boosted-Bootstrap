...
- In the second auto-column, we’ve reset the column count on the nested `.grid` to {asArray.find((token) => token.name === '$ouds-grid-3xl-column-count').compiledValue} (our default).
