...
- **Gutter utilities start at <code>{allTokens.find((token) => token.name === '$ouds-space-fixed-3xs').compiledValue}</code> and go up to <code>{allTokens.find((token) => token.name === '$ouds-space-fixed-5xl').compiledValue}</code> wide.** This allows us to match our grid to the [padding and margin spacers]([[docsref:/utilities/spacing]]) scale.
...
<Example class="bd-example-cols" code={`<div class="container-fluid px-2xlarge text-center">
...
<Example class="bd-example-cols" code={`<div class="container-fluid text-center">
    <div class="row row-cols-2 row-cols-lg-5 g-small g-lg-tallest">
      <div class="col">
