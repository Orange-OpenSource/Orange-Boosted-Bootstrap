...
- **Gutter utilities start at <code>{asArray.find((token) => token.name === '$ouds-space-fixed-3xs').compiledValue}</code> and go up to <code>{asArray.find((token) => token.name === '$ouds-space-fixed-5xl').compiledValue}</code> wide.** This allows us to match our grid to the [padding and margin spacers]([[docsref:/utilities/spacing]]) scale.
...
<Example class="bd-example-cols" code={`<div class="container-fluid px-2xl text-center">
