...
Use the `margin` and `padding` [spacing utilities]([[docsref:/utilities/spacing]]) to control how elements and components are spaced and sized. OUDS Web includes an eleven-level scale for spacing utilities, based on a <code>{allTokens.find((token) => token.name === '$core-ouds-dimension-base').compiledValue}</code> value default <code>{allTokens.find((token) => token.name === '$core-ouds-dimension-base').name}</code> variable. Choose values for all viewports (e.g., `.me-lg` for <code>margin-right: {allTokens.find((token) => token.name === '$ouds-space-fixed-large').compiledValue}</code> in LTR), or pick responsive variants to target specific viewports (e.g., `.me-md-lg` for <code>margin-right: {allTokens.find((token) => token.name === '$ouds-space-fixed-large').compiledValue}</code> —in LTR— starting at the `md` breakpoint).
