...
If the control items are checkboxes or radio buttons, selecting a row will automtically apply some styles to visually indicate that the row is selected. If you want to manually apply the same styles to rows or even cells, you can use the class `.table-active` on the `<tr>` or `<td>` elements.


to visually indicate that a row is selected (except the header row), add the `.table-active` class to the `<tr>` element. This applies a background color to highlight the selected row, ensuring users can easily identify which rows are currently selected.
