...
To display a helper link below inputs, use a standard small link [with `.link` and `.link-small` classes]([[docsref:/components/link#sizes]]) as a sibling of a `.text-input-container`.
...
To display an invalid input, add `.is-invalid` to a `.text-input-field` within the `.text-input-container`. Please take a look at our [Validation page to learn more]([[docsref:/components/validation]]).
