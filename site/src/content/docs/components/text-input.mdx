...
To indicate that a text input is mandatory in a form, add the class `.is-required` to display a red asterisk at the end of the label.

For general rules about when to add mandatory fields indication, please refer to the [text input design guidelines](https://r.orange.fr/r/S-ouds-doc-text-input) and to the [form validation documentation]([[docsref:/foundation/form-validation#mandatory-fields-indication]]). In particular, do not forget to handle error messages.
