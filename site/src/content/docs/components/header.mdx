...
<Callout type="danger">
  The header component is not yet designed, and this component is only a placeholder for the moment. It will be updated in the future with the final design and will certainly need adjustments regarding the DOM. Use it as a temporary component to speed up your development.

  The documentation only reflects an approximate header. Please refer to the [Orange Navbar in the Boosted documentation](https://boosted.orange.com/docs/components/orange-navbar/) to have a full view of the header component and its variations. You’ll only need to adapt the utilities inside the DOM.
</Callout>

## Global header

Here is a [simple header example](https://boosted.orange.com/docs/components/orange-navbar/#standard-1) with a logo on the left. The header is responsive and will collapse on smaller screens.

<Example class="p-none" code={`<header${getConfig().brand.includes('orange') ? ' data-bs-theme="dark"' : ''}>
...
Here is a [simple supra bar example](https://boosted.orange.com/docs/components/orange-navbar/#supra-bar). The supra bar is placed before the global header.

<Example class="p-none" code={`<nav class="navbar navbar-expand-lg supra"${getConfig().brand.includes('orange') ? ' data-bs-theme="dark"' : ''} aria-label="Supra navigation - With an additional languages navbar example">
    <div class="container-fluid container-max-width">
...
Here is a [simple navigation under example](https://boosted.orange.com/docs/components/orange-navbar/#navigation-under). The navigation under is placed after the global header.
