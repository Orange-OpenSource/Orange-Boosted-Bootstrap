...
They don't come with any height, but we provide some helpers to help to set the height of some skeletons.
...
Use `.skeleton-title` to render a skeleton with a height of medium title on itself and for children elements.

<Callout type="info">
  Titles should be represented on one full line.
</Callout>
...
Use `.skeleton-text` to render a skeleton with a height of medium paragraph on itself and for children elements.
...
To set components in skeleton state, you can wrap any of them in an `[aria-busy="true"]` container, and add the `inert` attribute to it. This will set all the children components in skeleton state, and make them unaccessible.
