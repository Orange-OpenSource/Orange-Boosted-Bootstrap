...
<Callout type="info">
The recommended way of using an icon in a button is through an [SVG sprite file]([[docsref:/extend/icons#svg-sprite]]). You need to fill it using `currentColor` to respect the button color scheme (which can be set directly inside the SVG sprite).
</Callout>
...
If really needed, you can use a [Web font]([[docsref:/extend/icons#web-font]]) associated to the `.icon` class to set correct parameters for the `font-size` and `line-height`. However, we strongly advise against using an `<img />` tag, as the icon will not benefit from dynamic color changes on states (hover, focus, active) and the color mode system will not work.
