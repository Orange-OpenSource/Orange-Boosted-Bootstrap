...
<ul class="list-unstyled row row-cols-1 md:row-cols-3">
  ...
  <li class="col">
    <div class="card position-relative h-100">
      ...
      <div class="card-body border-default border-thin border-top"><a class="stretched-link" href="#assistant-button">Assistant button</a> is a UI element that triggers an AI-related action or event.</div>
    </div>
  </li>
</ul>
...
{/* TODO: udpate link when the assistant button design spec is online https://github.com/Orange-OpenSource/Orange-Boosted-Bootstrap/issues/3734 */}
Button's width won't exceed <code>{(getTokenValue('$core-ouds-dimension-4000').slice(0, -2)) / 16}rem</code> and will automatically wrap if necessary. Howerver, buttons natively support `<br/>` tags to break the text into multiple lines when needed. Read more about multiline and responsiveness in a [button](https://unified-design-system.orange.com/472794e18/p/689c55-button/t/b29aae8b37), [navigation button](https://unified-design-system.orange.com/472794e18/p/1469c4-navigation-button/t/99e6bc9526) and [assistant button](https://unified-design-system.orange.com/472794e18/p/689c55-button/t/b29aae8b37).

<Example buttonLabel="buttons with wrapping text" class="bd-btn-example d-flex flex-column align-items-start" code={`<button type="button" class="btn btn-default">
    A button will automatically wrap if the text is longer than the default max width, or if the container is not large enough
  </button>

  <button type="button" class="btn btn-default">
    A button<br/>will automatically wrap if the text is longer than the default max width, but you can force it where you want
  </button>

  <button type="button" class="btn btn-assistant">
    The label remains centered with the icon when it wraps because of the max width
  </button>

  <button type="button" class="btn btn-assistant">
    The label<br/>remains centered with the icon when you force a break line too
  </button>`} />

 ### Block buttons

Create responsive stacks of "block buttons" with a mix of our display and gap utilities. By using utilities, we have great control over spacing, alignment, and responsive behaviors.
...
You can adjust the width of your block buttons with grid column width classes like `.col-3`. Center it horizontally with `.mx-auto`, too. Buttons will always be limited by their max-width, so they won't stretch too much on large screens.

<Example buttonLabel="half-width block buttons" class="bd-btn-example" code={`<div class="d-grid gap-small col-3 md:col-8 mx-auto">
