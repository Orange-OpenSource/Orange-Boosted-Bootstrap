...
{/* TODO: update link when the assistant button design spec is online https://github.com/Orange-OpenSource/Orange-Boosted-Bootstrap/issues/3734 */}
<Callout type="info">
...
<ul class="list-unstyled grid gap-medium">
  ...
  <li class="g-col-6">
    <div class="card position-relative h-100">
      <div class="card-top bg-secondary">
        <div class="d-flex align-items-center justify-content-center ratio-16x9" inert>
          <button type="button" class="btn btn-expand btn-default">Expand</button>
        </div>
      </div>
      <div class="card-body border-default border-thin border-top"><a class="stretched-link" href="#expand-button">Expand button</a> is a UI element that toggles content visibility, allowing users to expand or collapse content sections.</div>
    </div>
  </li>
  <li class="g-col-6">
    <div class="card position-relative h-100">
...
<Example showToolbar={false} code={`<div class="dropdown">
    <button type="button" class="btn btn-default btn-expand" data-bs-toggle="dropdown" aria-expanded="false">Expand</button>
...
<Callout type="warning">
  Icons in text and icon buttons are considered purely decorative, so they should be hidden from assistive technologies using `aria-hidden="true"`, as we’ve done in our examples. See [icons accessibility]([[docsref:/foundation/icons#accessibility]]) for more info.
</Callout>
...
<Callout type="warning">
  For icon only buttons, we chose to keep the icon hidden from assistive technologies and add a visually hidden label. See [icons accessibility]([[docsref:/foundation/icons#accessibility]]) for more info.
</Callout>
...
Buttons are available in two sizes. Add `.btn-small` to have a smaller button.
...
<Example buttonLabel="block buttons until the md breakpoint" class="bd-btn-example" code={`<div class="d-grid gap-small md:d-block">
...
<Example buttonLabel="right aligned inline buttons" class="bd-btn-example" code={`<div class="d-grid gap-small md:d-flex md:justify-content-end">

    <button class="btn btn-default md:me-xsmall" type="button">Button</button>
...
A button's text should not exceed one line, but will automatically wrap if necessary. It also natively supports `<br/>` tags to break the text into multiple lines. Read more about [text wrapping in a button](https://r.orange.fr/r/S-ouds-doc-button/t/b29aae8b37).

If you don’t want the button text to wrap, you can add the `.text-nowrap` class to the button. In Sass, you can set `$btn-white-space: nowrap` to disable text wrapping for all buttons.

<Example buttonLabel="buttons with wrapped text" class="bd-btn-example d-flex flex-column align-items-start" code={`<button class="btn btn-default" type="button">
    <svg aria-hidden="true">
      <use xlink:href="${getVersionedDocsPath('/assets/img/ouds-web-sprite.svg#heart-empty')}"/>
    </svg>
    The label remains centered in<br/>multiline mode with an icon
  </button>
  ...
  <button class="btn btn-default" type="button">
    ...
    In small contexts the button text wraps automatically to adapt its behavior
  </button>`} />
...
<Example buttonLabel="disabled navigation buttons using `<a>` tag" class="bd-btn-example" code={`<a class="btn btn-next btn-default" aria-disabled="true">Next</a>
...
<Example buttonLabel="disabled navigation buttons using `<button>` tag" class="bd-btn-example" code={`<button type="button" class="btn btn-next btn-default" disabled>Next</button>
...
<Example buttonLabel="loading navigation buttons" class="bd-btn-example" code={`<a class="btn btn-next btn-default loading-indeterminate" id="loading-nav-btn-1" aria-disabled="true">
    Next
    <svg viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg' class="loader" aria-hidden="true">
      <circle class="loader-inner" cx="20" cy="20" r="17"></circle>
    </svg>
...
<Example buttonLabel="navigation buttons with wrapped text" class="bd-btn-example d-flex flex-column align-items-start" code={`<button class="btn btn-default btn-next" type="button">
    The label remains centered in<br/>multiline mode with an icon
  </button>

  <button class="btn btn-default btn-previous" type="button">
    In small contexts the button text wraps automatically to adapt its behavior
  </button>`} />

## [[comp]] Expand button

An expand button is a UI element that toggles content visibility, allowing users to expand or collapse content sections. The expand button minimizes visual clutter by displaying only essential information and allowing access to additional details when necessary.

You can use `.btn-expand` class to indicate a [dropdown menu]([[docsref:/components/dropdown]]) can be expanded. This class will add an icon after the button text. As expand buttons generally indicate a dropdown menu, in our examples, we have used `<button>` elements, but this could be adapted to your own context. Note that the icon is changed automatically depending on the `[aria-expanded]` attribute.

<Callout type="negative" title="Dropdown menu">
  Be aware that the dropdown menu is not part of the expand button component and is a draft component. Please refer to the [dropdown menu documentation]([[docsref:/components/dropdown]]) for more information.
</Callout>

<Example buttonLabel="basic expand button" class="bd-btn-example" code={`<div class="dropdown">
    <button type="button" class="btn btn-default btn-expand" data-bs-toggle="dropdown" aria-expanded="false">Expand</button>
    <ul class="dropdown-menu">
      <li><button type="button" class="dropdown-item">Action</button></li>
      <li><button type="button" class="dropdown-item">Another action</button></li>
...
OUDS Web includes several expand button variants, each serving its own semantic purpose. Please follow the [expand button design specifications](https://r.orange.fr/r/S-ouds-doc-expand-button) to choose the right button variant for the right action.

<Callout type="warning" title="Beware">
  The button and the expand button have the same variants except for the negative, which does not exist for the expand button.
</Callout>

<Example buttonLabel="expand buttons variants" class="bd-btn-example" code={`<div class="dropdown">
    <button type="button" class="btn btn-default btn-expand" data-bs-toggle="dropdown" aria-expanded="false">Expand</button>

    <button type="button" class="btn btn-strong btn-expand" data-bs-toggle="dropdown" aria-expanded="false">Expand</button>

    <button type="button" class="btn btn-brand btn-expand" data-bs-toggle="dropdown" aria-expanded="false">Expand</button>

    <button type="button" class="btn btn-minimal btn-expand" data-bs-toggle="dropdown" aria-expanded="false">Expand</button>
...
<Example buttonLabel="icon only expand button" class="bd-btn-example" code={`<div class="dropdown">
    <button type="button" class="btn btn-default btn-icon btn-expand" data-bs-toggle="dropdown" aria-expanded="false">
      <svg aria-hidden="true">
        <use xlink:href="${getVersionedDocsPath('/assets/img/ouds-web-sprite.svg#heart-empty')}"/>
      </svg>
      <span class="visually-hidden">Favorite playlist</span>
    </button>
    <ul class="dropdown-menu">
      <li><button type="button" class="dropdown-item">Action</button></li>
      <li><button type="button" class="dropdown-item">Another action</button></li>
...
<BrandSpecific brand={['orange', 'orange-compact']}>
  ...
  Rounded corners expand buttons is a project-wide option therefore all your buttons should either be rounded or not. Add `.use-rounded-corner-buttons` on a top container (for example `<body>`) to use rounded corners.
...
<Example buttonLabel="rounded expand buttons" class="bd-btn-example" code={`<div class="use-rounded-corner-buttons">
    <div class="dropdown">
      <button type="button" class="btn btn-default btn-expand" data-bs-toggle="dropdown" aria-expanded="false">Expand</button>
      <ul class="dropdown-menu">
        <li><button type="button" class="dropdown-item">Action</button></li>
...
Make expand buttons inactive by adding the `[disabled]` boolean attribute to the `<button>` element. Disabled expand buttons have `pointer-events: none` applied to, preventing hover and active states from triggering.

<Example buttonLabel="disabled expand button" class="bd-btn-example" code={`<div class="dropdown">
    <button type="button" class="btn btn-default btn-expand" data-bs-toggle="dropdown" aria-expanded="false" disabled>Expand</button>
    <ul class="dropdown-menu">
      <li><button type="button" class="dropdown-item">Action</button></li>
      <li><button type="button" class="dropdown-item">Another action</button></li>
      <li><button type="button" class="dropdown-item">Something else here</button></li>
...
The loading state of an expand button indicates that an action is currently processing or taking place. This state provides feedback to users, enhancing user experience.

Please refer to the [loading state section of the button component](#loading) for guidelines on how to implement loading states, as well as a JavaScript example and a live demo.

<Example buttonLabel="loading expand buttons" class="bd-btn-example" code={`<div class="dropdown">
    <button type="button" class="btn btn-default btn-expand loading-indeterminate" data-bs-toggle="dropdown" aria-expanded="false" disabled>
      Expand
      <svg viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg' class="loader" aria-hidden="true">
        <circle class="loader-inner" cx="20" cy="20" r="17"></circle>
      </svg>
      <span role="status" id="loading-expand-btn-msg-1" class="visually-hidden">Loading expand</span>
    </button>
    <ul class="dropdown-menu">
      <li><button type="button" class="dropdown-item">Action</button></li>
      <li><button type="button" class="dropdown-item">Another action</button></li>
...
<SkeletonRedirect component="expand button" />

<Example buttonLabel="expand button skeleton" code={`<div aria-busy="true" inert>
    <div class="dropdown">
      <button type="button" class="btn btn-default btn-expand" data-bs-toggle="dropdown" aria-expanded="false">Expand</button>
      <ul class="dropdown-menu">
        <li><button type="button" class="dropdown-item">Action</button></li>
...
A button's text should not exceed one line, but will automatically wrap if necessary. It also natively supports `<br/>` tags to break the text into multiple lines. Read more about [text wrapping in a button](https://r.orange.fr/r/S-ouds-doc-expand-button/t/2bd515e9b8).

If you don’t want the button text to wrap, you can add the `.text-nowrap` class to the button. In Sass, you can set `$btn-white-space: nowrap` to disable text wrapping for each button.

<Example buttonLabel="expand buttons with wrapped text" class="bd-btn-example d-flex flex-column align-items-start" code={`<div class="dropdown">
...
An assistant button is a UI element that triggers an action or event featuring AI-powered capabilities, such as conversational assistance, content generation, or contextual suggestions.

The assistant button combines standard button functions with AI-specific visual features to clearly indicate actions enhanced or powered by artificial intelligence.
...
<Example buttonLabel="basic assistant buttons" class="bd-btn-example" code={`<a class="btn btn-assistant" href="#">Assistant</a>

  <button type="button" class="btn btn-assistant">Assistant</button>`} />

<Callout type="warning" title="Transparency and older browsers">
  An assistant button does not have a transparent background on older browsers and Firefox because [`background-clip: border-area` is not fully supported](https://caniuse.com/wf-background-clip-border-area).

  It's necessary to set `--bs-btn-bg` to its container background color to be compatible with most browsers. Make sure that the used variable isn't semi-transparent (ie. use background tokens instead of surface tokens).
</Callout>
...
Add the class `.btn-icon` to get an icon-only assistant button. Make sure to provide an accessible name to your button, either using a `.visually-hidden` content or an `aria-label` attribute. Icon-only assistant buttons are always completely circular.

<Example buttonLabel="assistant button icon-only" class="bd-btn-example" code={`<button type="button" class="btn btn-assistant btn-icon">
...
<ThemeSpecific theme={['orange', 'orange-compact']}>
  ...
  Rounded corners assistant buttons is a project-wide option therefore all your buttons should either be rounded or not. Add `.use-rounded-corner-buttons` on a top container (for example `<body>`) to use rounded corners.
...
<Example buttonLabel="rounded assistant buttons" class="bd-btn-example" code={`<div class="use-rounded-corner-buttons">
...
<Example buttonLabel="disabled assistant buttons using `<button>` tag" class="bd-btn-example" code={`<button type="button" class="btn btn-assistant" disabled>Assistant</button>

  <button type="button" class="btn btn-assistant btn-icon" disabled>
    <span class="visually-hidden">Assistant</span>
  </button>`} />
...
<Example buttonLabel="loading assistant buttons" class="bd-btn-example" code={`<button type="button" class="btn btn-assistant loading-indeterminate" id="loading-ai-btn-1" disabled>
    Assistant
    <svg viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg' class="loader" aria-hidden="true">
      <circle class="loader-inner" cx="20" cy="20" r="17"></circle>
    </svg>
  ...
  <button type="button" class="btn btn-assistant btn-icon loading-indeterminate" id="loading-ai-btn-2" disabled>
    <svg viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg' class="loader" aria-hidden="true">
      <circle class="loader-inner" cx="20" cy="20" r="17"></circle>
    </svg>
    <span role="status" id="loading-ai-btn-msg-2" class="visually-hidden">Loading answer</span>
  </button>`} />
...
<Example buttonLabel="assistant button skeleton" code={`<div aria-busy="true" inert>
...
Assistant buttons are available in two sizes. Add `.btn-small` to have a smaller button.

<Example buttonLabel="small assistant buttons" class="bd-btn-example" code={`<button type="button" class="btn btn-assistant btn-small">Assistant</button>

  <button type="button" class="btn btn-assistant btn-icon btn-small">
    <span class="visually-hidden">Assistant</span>
  </button>`} />
...
{/* TODO: udpate link when the design spec is online https://github.com/Orange-OpenSource/Orange-Boosted-Bootstrap/issues/3734 */}
A button's text should not exceed one line, but will automatically wrap if necessary. It also natively supports `<br/>` tags to break the text into multiple lines. Read more about [text wrapping in a button](https://r.orange.fr/r/S-ouds-doc-navigation-button/t/99e6bc9526).

If you don’t want the button text to wrap, you can add the `.text-nowrap` class to the button. In Sass, you can set `$btn-white-space: nowrap` to disable text wrapping for each button.

<Example buttonLabel="assistant buttons with wrapped text" class="bd-btn-example d-flex flex-column align-items-start" code={`<button type="button" class="btn btn-assistant">
    The label remains centered in<br/>multiline mode with an icon
  </button>
