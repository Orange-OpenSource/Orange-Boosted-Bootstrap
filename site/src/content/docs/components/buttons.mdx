...
<Callout type="warning">
  Icons in text and icon buttons are considered purely decorative, so they should be hidden from assistive technologies using `aria-hidden="true"`, as we’ve done in our examples. See [icons accessibility]([[docsref:/foundation/icons#icons-accessibility]]) for more info.
</Callout>
...
<Callout type="warning">
  For icon only buttons, we chose to keep the icon hidden from assistive technologies and add a visually hidden label. See [icons accessibility]([[docsref:/foundation/icons#icons-accessibility]]) for more info.
</Callout>
