...
Use an `<a>` element only for navigation purpose, it must have an `href` attribute to make it focusable. For the [navigation button](#navigation-button), we have used `<a>` elements in our examples, but this could be adapted to your own context.
