...
<Callout type="info">
The recommended way of using an icon in a link is through an [SVG sprite file]([[docsref:/components/icon#svg-sprite]]). You need to fill it using `currentColor` to respect the link color scheme (can be set directly inside the SVG sprite).
</Callout>
