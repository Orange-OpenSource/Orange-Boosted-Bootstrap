...
This example shows how to implement a 'Show password' button for a password input field. This is achieved by toggling three elements:
  - the input type, changing it from `password` to `text`
  - the `aria-pressed` state of the button
  - the button icon

The visually hidden label of the button should not be changed as the `aria-pressed` state already indicates whether the button will show or hide the password. Please note that the button's `aria-pressed` state must be initialized. The JavaScript code provided below can be adapted for your project.
