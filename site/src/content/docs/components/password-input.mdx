...
This example shows how to implement a password input with a show/hide button that toggles the password visibility (by changing the input type to `text`) and updates the button icon. For security reasons, don't forget to reset the input type to `password` before form submission to prevent the password from being stored in browser autocomplete history.

TODO:
  - decide if we use data-bs-toggle or not - check the impact on styles since it adds the `.active` class to the button when toggled
  - decide which label to use (always show password or show/hide password)
...
<Example buttonLabel="show password live example" addStackblitzJs code={`<div id="liveShowPasswordExample" class="text-input component-max-width">
    <div class="text-input-container">
      ...
      <input type="password" id="liveInputPassword" class="text-input-field" placeholder=" ">
        <button class="btn btn-minimal btn-icon" id="togglePassword" data-bs-toggle="button" aria-pressed="false" type="button">
          <svg aria-hidden="true">
            <use xlink:href="${getVersionedDocsPath('/assets/img/ouds-web-sprite.svg#accessibility-vision')}"/>
          </svg>
          <span class="visually-hidden">Show password</span>
