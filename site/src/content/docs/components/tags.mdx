...
For accessibility reasons, any informative element, must either be a semantic element itself or be wrapped in an appropriate semantic HTML element. In the examples below, the tags appear as `<li>` elements or inside of `<h2>` or `<p>` elements. The input tags are implemented as `<button>` elements. You may use any semantic element that best fits the context.

Keep in mind that visually impaired and blind users will lack the visual context (color for instance) that permits to understand the tags purpose on the page, if tags' labels are not self-explanatory. In thas case, it's mandatory to add textual information to explain what it represents, especially if the tags are in a list.

*For tag and input tag lists: add a semantically informative text to describe the list purpose* if possible, otherwise an `aria-label` on the containing `<ul>` or `<ol>` must be added.
...
<Example buttonLabel="default tags" code={`<ul class="list-unstyled d-flex column-gap-xsmall" aria-label="Mobile coverage">
...
Tags have status depending on the context of the information they represent. Each state is designed to convey a specific meaning and ensure clarity in communication. Their appearance can be emphasized (by default, for key tags) or muted (by adding `.tag-muted`, for secondary tags).

##### Non-functional

Non-functional tags are used to display categories, default states, or to draw attention without carrying a specific functional meaning (unlike functional tags, see juste below).
...
<Callout type="warning">
  Keep in mind that color should not be the only way to convey information. If the color has strong meaning, reflect it in the tag's text and if necessary add additional text with the class `.visually-hidden`. [Read more about tags accessibility here](#accessibility).
</Callout>
...
As functional tags communicate specific statuses, each status must always be paired with its dedicated functional icon that matches the meaning of the tag, and other icons must not be used. To display a status icon, just add `.tag-status-icon` inside the tag.
