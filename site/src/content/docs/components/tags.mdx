...
When inserting tags into a list, wrap the list in a semantic container (usually a `<ul>` or `<ol>`). If the purpose of the tags is not clear enough (especially without visual context), an `aria-label` must be added to the container to explain the function of the tags.
</Callout>
