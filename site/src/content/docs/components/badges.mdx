...
If the badge is in a disabled context (for example, inside a disabled button), the badge will automatically take the disabled state without needing to add the `.disabled` class. This works if the ancestor element has the `[disabled]` attribute, or the `.disabled` class.
