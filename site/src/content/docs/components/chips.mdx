...
For visually impaired and blind users, the visual context is inaccessible. Thus, they cannot understand the purpose of the chips on the page. So it's mandatory to add a hidden text (or an `aria-label` on the containing `ul` for radio buttons) to explain the use of chips without context. Don’t forget to adapt and translate this text to your use case.
