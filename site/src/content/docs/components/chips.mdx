...
<ul class="list-unstyled row row-cols-1 sm:row-cols-3">
  <li class="col">
    <div class="card position-relative h-100">
      <div class="card-top bg-secondary" inert>
      ...
      <div class="card-body border-default border-thin border-top"><a class="stretched-link" href="#filter-chip">Filter chip</a> is a UI element that enables users to select or deselect options within a series, commonly used to capture filtering decisions.</div>
    </div>
  </li>
  <li class="col">
    <div class="card position-relative h-100">
      <div class="card-top bg-secondary" inert>
        <div class="d-flex align-items-center justify-content-center ratio-16x9">
          <ul class="chips-container overflow-visible">
            <li class="chip chip-filter dropdown">
              <button class="chip-interactive" data-bs-toggle="dropdown" aria-expanded="false" type="button">
                <span>Expand</span>
                <span class="chip-badge-chevron">
                  <span class="badge badge-neutral badge-count">1<span class="visually-hidden">filter(s) selected</span></span>
                </span>
              </button>
            </li>
          </ul>
      ...
      <div class="card-body border-default border-thin border-top"><a class="stretched-link" href="#filter-chip-expand">Filter chip expand</a> is a UI element that allows users to control the selection or deselection of a choice from a defined list of options, frequently used to capture filtering decisions.</div>
    </div>
  </li>
  <li class="col">
    <div class="card position-relative h-100">
      <div class="card-top bg-secondary" inert>
        <div class="d-flex align-items-center justify-content-center ratio-16x9">
          <p class="chip chip-suggestion mb-none">
            <button class="chip-interactive" tabindex="-1" type="button">Suggestion</button>
          </p>
        </div>
      </div>
      <div class="card-body border-default border-thin border-top"><a class="stretched-link" href="#suggestion-chip">Suggestion chip</a> is a UI element that presents recommended or predictive options based on the user's input or context, often utilised to capture filtering decisions.</div>
    </div>
  </li>
</ul>
...
They are based on checkbox inputs, but we also have [button based](#using-buttons) or [radio button based](#using-radio-buttons) filter chips.
...
<Example buttonLabel="filter chips using radio buttons" code={`<ul class="chips-container" aria-label="Filter by">
    <li class="chip chip-filter">
...
<Example buttonLabel="text and icon filter chips" code={`<ul class="chips-container" aria-label="Filter by">
    <li class="chip chip-filter">
...
<Example buttonLabel="icon only filter chips" code={`<ul class="chips-container" aria-label="Filter by">
    <li class="chip chip-filter">
...
<Example buttonLabel="disabled filter chips" code={`<ul class="chips-container" aria-label="Filter by">
    <li class="chip chip-filter">
...
<SkeletonRedirect component="filter chip" />

<Example buttonLabel="filter chip skeleton" code={`<div aria-busy="true" inert>
    <ul class="chips-container">
      <li class="chip chip-filter">
...
<Code buttonLabel="JavaScript to toggle all chips" lang="js" code={`document.querySelectorAll('.chip-suggestion').forEach(chipElement => {
    const chip = oudsWeb.Button.getOrCreateInstance(chipElement)
    chip.toggle()
  })`} />

## [[comp]] Filter chip expand

A filter chip expand is a UI element that allows users to control the selection or deselection of a choice from a defined list of options, frequently used to capture filtering decisions.

The filter chip expand feature is displayed through a dropdown with selectable values, making it ideal for filters with multiple or dynamic options while providing clarity and saving space in the interface.

To create a filter chip expand, add `.chip-filter.dropdown` to the `.chip` and `.chip-badge-chevron` inside the `.chip-interactive`.

The selected state of the filter chip expand is triggered by the presence either of a [Badge count]([[docsref:/components/badges#badge-count]]) not being `.d-none`. If needed, it's also possible to trigger it by adding `.active` to the `.chip-interactive`.

`.chip-badge-chevron` class indicates a [dropdown menu]([[docsref:/components/dropdown]]) can be expanded. This class will add a directional arrow to the left of the button text. As filter chip expand generally indicates a dropdown menu, in our examples, we have used `<button>` elements, but this could be adapted to your own context.
...
<Example buttonLabel="basic filter chip expand" code={`
  <h4 id="filterExpandHeader">Filters</h4>
  <div role="group" aria-labelledby="filterExpandHeader">
    <ul class="chips-container overflow-visible">
      <li class="chip chip-filter dropdown">
        <button class="chip-interactive" data-bs-toggle="dropdown" aria-expanded="false" type="button">
          Brands
...
Add `disabled` attribute either on the button element to disable the chip.

The following example shows every possible renderings depending on the filter chip type but it is not recommended to mix different types in a real use case.

<Example buttonLabel="disabled filter chip expand" code={`<ul class="chips-container overflow-visible" aria-label="Filters">
    <li class="chip chip-filter dropdown">
      <button class="chip-interactive" disabled data-bs-toggle="dropdown" aria-expanded="false" type="button">
        Brands
        <span class="chip-badge-chevron">
          <span class="badge badge-neutral badge-count">1<span class="visually-hidden">filter(s) selected</span></span>
        </span>
...
<SkeletonRedirect component="filter chip expand" />
...
This example shows how to implement a filter chip expand. This is achieved by toggling four elements:
  - The badge needs to be updated according to the number of selected filters. If no filter is selected, the badge should be hidden using `.d-none`.
  - The checked state inside each dropdown item should be toggled based on the `aria-pressed` state of the item. (This part might change when the dropdown menu component is finalized.)

<Callout type="negative" title="Dropdown menu">
  Be aware that the dropdown menu is not part of the filter chip expand component and is a draft component. Please refer to the [dropdown menu documentation]([[docsref:/components/dropdown]]) for more information.
</Callout>

<Example buttonLabel="filter chip expand live example" code={`<ul class="chips-container overflow-visible" aria-label="Filters">
    <li class="chip chip-filter dropdown" id="filterChipExpandExample">
      <button class="chip-interactive" data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false" type="button">
        Brands
        <span class="chip-badge-chevron">
          <span class="badge badge-neutral badge-count d-none">1 <span class="visually-hidden">filter(s) selected</span></span>
        </span>
      </button>
      <div class="dropdown-menu">
...
<JsDocs name="live-filter-chip-expand" file="site/src/assets/partials/snippets.js" />
...
<Example buttonLabel="basic suggestion chips" code={`<ul class="chips-container" aria-label="Answer with">
...
<Example buttonLabel="text and icon suggestion chips" code={`<ul class="chips-container" aria-label="Answer with">
    <li class="chip chip-suggestion">
      <button class="chip-interactive" type="button">
        <svg aria-hidden="true">
          <use xlink:href="${getVersionedDocsPath('/assets/img/ouds-web-sprite.svg#heart-empty')}" />
...
<Example buttonLabel="icon only suggestion chips" code={`<ul class="chips-container" aria-label="Answer with">
    <li class="chip chip-suggestion">
...
<Example buttonLabel="disabled suggestion chips" code={`<ul class="chips-container" aria-label="Answer with">
    <li class="chip chip-suggestion">
...
<SkeletonRedirect component="suggestion chip" />

<Example buttonLabel="suggestion chip skeleton" code={`<div aria-busy="true" inert>
    <ul class="chips-container">
      <li class="chip chip-suggestion">
        <button class="chip-interactive" type="button">
          Thanks.
...
A chip's text should not exceed one line. When needed, it also supports `<br/>` tags to break the text into multiple lines. Read more about [text wrapping in a filter chip](https://r.orange.fr/r/S-ouds-doc-filter-chip/t/75dd4ff373).

<Example buttonLabel="multiline filter chip" class="d-flex flex-wrap gap-medium" code={`<div class="chip chip-filter">
  ...
  <div class="chip chip-filter dropdown">
    <button class="chip-interactive" data-bs-toggle="dropdown" aria-expanded="false" type="button">
      Filter chip expand<br/>
      with a very long label
      <div class="chip-badge-chevron">
        <span class="badge badge-neutral badge-count">1<span class="visually-hidden">filter(s) selected</span></span>
      </div>
    </button>
    <div class="dropdown-menu">
