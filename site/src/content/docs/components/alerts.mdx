...
You may also want to use `<dialog>` element for your alert if it’s dismissible, it requires a user action and you have to display it like a modal. In this case, you can still use our alert classes to style the content of the dialog. See our [live example using `<dialog>`](#using-dialog) for more details.
...
- Or via `aria-label` and a descriptive text inside to know what the associated action is.
...
The variants `.alert-neutral` and `.alert-accent` are considered as non-functional.
...
For [non-functional alert messages](#non-functional), the displayed icon can be customized.

This can be achieved by using one of the following solutions:
* adding a `<svg>`, `<img>` or an `.icon` inside the `.alert-icon` container.
* applying a `mask-image` on `.alert-icon` with a class or an inline `style` attribute.
* setting the custom property `--bs-alert-icon` inside the `.alert`.
...
If the alert can be removed when its content is acknowledged, add a close button inside the `.alert-action-container` with our [button icon]([[docsref:/components/buttons#icon-only]]).

<Callout type="info">
  If you want the JavaScript behavior presented on the first alert below, please refer to our [JavaScript section](#javascript).
</Callout>

<Callout type="info" title="One line title alert">
  If you have a one line title-only `.alert-message` and you are using a rounded alert (that might happen in our documentation depending on the brand), you can wrap the `.btn-close` with an element holding `.alert-close-container` class to prevent issues with the rounded button. See it in action in our [live example](#live-example).
</Callout>

<Callout type="warning">
  For accessibility purpose, please provide a meaningful label for the close button. For example, adding an `aria-labelledby` linking to the title of the alert, or adding a `.visually-hidden` content.
</Callout>
...
You can add an action [link]([[docsref:/components/links]]) (`<a>` or `<button` depending on the context of use) either after the `.alert-text-container` or inside the `.alert-action-container`.
...
<Callout type="warning">
  Please note that the `.bullet-list` needs to have `.fs-bm` in order to change its bullet size, and that it should be only one level depth.
</Callout>
...
<BrandSpecific brand="orange">
  <Callout type="info">
    We use `.use-rounded-corner-alert` here to detect edge cases.
  </Callout>
</BrandSpecific>

<Example buttonLabel="alert message with live behavior" addStackblitzJs code={`<div id="liveAlertPlaceholder" role="status" class="${getConfig().brand === 'orange' ? 'use-rounded-corner-alert ' : '' }d-flex flex-column gap-medium mb-medium">
    <div class="alert alert-message alert-positive">
      <div class="alert-icon"></div>
      <div class="alert-container">
        <div class="alert-text-container">
...
This use case is for future friendly projects. Our Alert plugin might be removed from our JavaScript someday in favor of a more native approach.

You should  use `<dialog>` element for your alert only if it’s dismissible, it requires a user action and you have to display it like a modal. In this case, you can still use our alert classes to style the content of the dialog.

<Callout type="negative">
  The [disponibility of the Invoker Commands API](https://caniuse.com/wf-invoker-commands) is very limited for now, so we don't recommand using it for now.

  When using a `<dialog>` element, the alert will be displayed as a modal. This is because the [dialog’s modality](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/dialog) takes precedence over the alert’s CSS. There are 2 different displays for the alert message, depending on whether the `popover` attribute is used or not:

  <ul class="bullet-list fs-bm">
    <li>Without the `popover` attribute, the alert message will be displayed as a modal, without the possibility of clicking on the backdrop to dismiss, the backdrop removing all the pointer events and a focustrap on the modal.</li>
    <li>With the `popover` attribute, the alert message will be displayed as a modal, with the possibility of clicking on the backdrop to dismiss and no focustrap.</li>
  </ul>
</Callout>

<details class="mb-medium">
  <summary>See both in action</summary>
  <Example buttonLabel="live alert messages without JavaScript" class="d-flex flex-wrap justify-content-between gap-medium" code={`<dialog id="my-modal" class="alert alert-message alert-warning" popover>
      <div class="alert-icon"><p class="visually-hidden">Warning alert</p></div>
      <div class="alert-container">
        <div class="alert-text-container">
          <h5 class="alert-label">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed nec risus et risus consectetur dignissim volutpat ut lorem.</h5>
          <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed nec risus et risus consectetur dignissim volutpat ut lorem. Aenean posuere elementum massa, ac elementum magna auctor quis. Aliquam erat volutpat. Ut quam turpis, interdum non ex at, imperdiet ornare mi. Curabitur eget hendrerit diam. Curabitur id placerat ipsum.</p>
        </div>
        <a href="#" class="link">Action</a>
      </div>
      <div class="alert-action-container">
        <button class="btn-close" command="close" commandfor="my-modal">
          <span class="visually-hidden">Close warning alert</span>
        </button>
      </div>
    </dialog>

    <dialog id="my-popover" class="alert alert-message alert-warning" popover>
      <div class="alert-icon"><p class="visually-hidden">Warning alert</p></div>
      <div class="alert-container">
        <div class="alert-text-container">
          <h5 class="alert-label">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed nec risus et risus consectetur dignissim volutpat ut lorem.</h5>
      ...
      <div class="alert-action-container">
        <button class="btn-close" popovertarget="my-popover" popovertargetaction="hide">
          <span class="visually-hidden">Close warning alert</span>
        </button>
      </div>
    </dialog>

    <button command="show-modal" commandfor="my-modal" class="btn btn-default">Show modal alert</button>
    <button popovertarget="my-popover" class="btn btn-default">Show popover alert</button>`} />
</details>
...
<Callout type="info" title="Please note that this variant of alert needs an icon specified." />
