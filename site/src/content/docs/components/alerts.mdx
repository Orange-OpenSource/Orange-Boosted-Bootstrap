...
`.alert-close-container` is only useful when there are rounded corner alerts, with a title on one line, in order to help to round the `.btn-close` automatically. Since it's a very specific usage, it may be omitted. If present, this element should only wrap the `.btn-close` button.
...
<Callout type="info" title="One line title alert">
  If you have an `.alert-message` with only a title that might be on one line and you are using a rounded alert (that might happen in our documentation depending on the documentation), you can wrap the `.btn-close` with an element holding `.alert-close-container` class to handle the rounded effect automatically. See it in action in our [live example](#live-example).
</Callout>
    ...
    <div class="alert-action-container">
      <div class="alert-close-container">
        <button class="btn-close" data-bs-dismiss="alert" aria-labelledby="btn-close-alert closable-alert-accent" id="btn-close-alert">
          <span class="visually-hidden">Close accent alert</span>
        </button>
      </div>
    </div>
  </div>
