...
This use case is for **future friendly projects only**. Our Alert plugin will probably be removed from our JavaScript someday in favor of [a more native approach](https://developer.mozilla.org/en-US/docs/Web/API/Invoker_Commands_API).
...
<Callout type="negative">
  ...
  With the `popover` attribute:

  <ul class="bullet-list fs-bm">
    <li>The alert message will be displayed as a modal, with the possibility of clicking on the backdrop to dismiss and no focustrap.</li>
    <li>To avoid it, we hide the popover when the alert is in shown state and we display it when the alert is in hidden state. The alert is hidden by default.</li>
  </ul>
</Callout>

<Example buttonLabel="live alert messages without JavaScript" class="d-flex flex-wrap justify-content-between gap-medium" code={`<div id="my-invoked-alert" class="alert alert-message alert-warning" popover>
    <div class="alert-icon"><p class="visually-hidden">Warning alert</p></div>
    <div class="alert-container">
      <div class="alert-text-container">
    ...
    <div class="alert-action-container">
      <button class="btn-close" popovertarget="my-invoked-alert" popovertargetaction="show">
        <span class="visually-hidden">Close warning alert</span>
      </button>
    </div>
  </div>

  <button popovertarget="my-invoked-alert" class="btn btn-default">Toggle invoked alert</button>`} />
