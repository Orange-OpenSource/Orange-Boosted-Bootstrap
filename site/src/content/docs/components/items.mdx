...
1. [Leading container](#leading-container) (optional) using `.item-leading-container`
2. [Text container](#text-container) (required) using `.item-text-container`
3. [Trailing container](#trailing-container) (optional) using `.item-trailing-container`
...
1. Overline (optional) using `.item-overline` (only if not [small item](#sizes))
2. Label (required) using `.item-label`, that can optionally be bold (using `.fw-bold`)
3. Extra label (optional) using `.item-extra-label` (only if not [small item](#sizes))
...
<Callout type="warning">
  ...
  * The assets within the [leading container](#leading-container) and the [trailing container](#trailing-container) apart tags and badges mustn't use any size classes
  * There mustn't be any [slot](#slots) in the item
  * The [text container](#text-container) must only have the label, and/or description inside and not the overline, or extra label
</Callout>
