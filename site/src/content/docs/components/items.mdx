...
Optionally, if the item is a navigation item and a slot is mandatory and also interactive, you can add `.item-slot-focusable` to the slot or the interactive element.<ThemeSpecific theme={['orange', 'orange-compact']}> Please note that the slot won't be [rounded](#rounded-corners) anymore.</ThemeSpecific>
...
<Callout type="info" title="Accessibility">
  Screen readers will read the entire item as a link. You can use the `.visually-hidden` text to provide additional context if needed. You won't need any `[aria-labelledby]` as stated in our [accessibility section](#accessibility).
...
<Callout type="negative">
  The interactive slot should be avoided inside the navigation item as much as possible. Prefer to have only Navigation item or multiple links inside a static item. So here, just like in the [alignment section](#alignment).
</Callout>
