...
- <span class="tag tag-small tag-negative"><span class="tag-status-icon"></span>Breaking</span> Alert message action links must now use [their compact version]([[docsref:/components/links#sizes]]) when they are included at the bottom of an alert message component. Read more in our [alerts page]([[docsref:/components/alerts#with-action]]).

- <span class="tag tag-small tag-info"><span class="tag-status-icon"></span>Info</span> Alert message component has been updated to 1.2.0. Read more in the [alert message design changelog](https://r.orange.fr/r/S-ouds-doc-alert-message_changelog).
