...
- <span class="tag tag-small rounded-none tag-positive">New</span> Alert message and inline alert has been implemented. Read more in our [alerts page]([[docsref:/components/alertd]]).
...
- <span class="tag tag-small rounded-none tag-negative">Breaking</span> `.ratio` has been replaced by the aspect-ratio based `.ratio-*` utilities. These utilities are applied directy on the target element, a container is no longer needed. Read more about [aspect ratio]([[docsref:/utilities/aspect-ratio]]).
