...
- <span class="tag tag-small rounded-none tag-warning">Warning</span> OUDS Web now supports an ersatz of header. It comes from Boosted directly with small tweaks and intend to change in the future. It's only a placeholder for sites that need it, but it is not intended to be a final solution. Read more in our [header page]([[docsref:/components/header]]).
