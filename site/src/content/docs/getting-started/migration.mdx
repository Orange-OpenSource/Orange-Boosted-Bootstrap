...
<Callout type="info">
  The [`@ouds/web-migrate`](https://npmx.dev/package/@ouds/web-migrate) CLI can help update from one version to another. Run it against your project files and review the diff afterward:
...
- <span class="tag tag-small tag-positive"><span class="tag-status-icon"></span>New</span> We have created a new CLI tool to update your project files automatically. Read more in our [CLI tool readme](https://www.npmx.dev/package/@ouds/web-migrate).

### Utilities

#### Breakpoint prefixes

- <span class="tag tag-small tag-negative"><span class="tag-status-icon"></span>Breaking</span> Responsive utilities now uses a breakpoint prefix instead of an infix. All classes that previously used `.{utility}-{breakpoint}-{value}` must be updated to `.{breakpoint}:{utility}-{value}`. Following are some examples of the changes to operate:
  - `.d-sm-none` → `.sm:d-none`
  - `.d-md-flex` → `.md:d-flex`
