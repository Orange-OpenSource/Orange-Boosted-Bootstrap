...
- <span class="tag tag-small tag-negative"><span class="tag-status-icon"></span>Breaking</span> Text area component has been updated to 1.2.0:
  - the placeholder attribute is now mandatory
  - a `.text-area-no-resize` class has been added to disable the resizing feature on the text area
  Read more in our [text area page]([[docsref:/components/text-area]]) and in the [design changelog](https://r.orange.fr/r/S-ouds-doc-text-area_changelog).
