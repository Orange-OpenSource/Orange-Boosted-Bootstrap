...
- <span class="tag tag-small tag-negative"><span class="tag-status-icon"></span>Breaking</span> Alert message action links must now use [their compact version]([[docsref:/components/links#sizes]]) when they are included at the bottom the an alert message. Read more in our [alerts page]([[docsref:/components/alerts#with-action]]).
