...
- <span class="tag tag-small tag-negative"><span class="tag-status-icon"></span>Breaking</span> `.pe-none` and `.pe-auto` classes have been deprecated in favor of `.pointer-events-none` and `.pointer-events-auto` to avoid collision with padding utilities. They will be removed in v1.6.0. Read more in our [interactions page]([[docsref:/utilities/interaction#pointer-events]]).
