...
- <span class="tag tag-small rounded-none tag-info">Info</span> `.icon-link` and `.link-chevron` are now documented in link component. See [icon link]([[docsref:/components/link#icon-link]]) and [link chevron]([[docsref:/components/link#link-chevron]]).
