...
- <span class="tag tag-small tag-negative"><span class="tag-status-icon"></span>Breaking</span> `.d-xxl-{value}` responsive display utility has been replaced by `.2xl:d-{value}`. Please refer to the [new breakpoints’ names]([[docsref:/layout/breakpoints#available-breakpoints]]). You can still have it using `$enable-bootstrap-compatibility`.
