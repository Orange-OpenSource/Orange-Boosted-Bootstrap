...
- <span class="tag tag-small rounded-none tag-negative">Breaking</span> Alerts have changed a lot. The DOM has changed a lot and we recommend to read our [new Alerts page]([[docsref:/components/alerts]]) first. `.alert-sm`, `.alert-success`, `.alert-danger`, `.alert-dismissible`, `.alert-heading` have been removed.
