...
- <span class="tag tag-small tag-negative"><span class="tag-status-icon"></span>Breaking</span> OUDS Web now uses [new functional icons]([[docsref:/foundation/icons#status-colors]]). A new [bicolor accessible warning icon]([[docsref:/foundation/icons#accessible-warning-icon]]) has been introduced.
