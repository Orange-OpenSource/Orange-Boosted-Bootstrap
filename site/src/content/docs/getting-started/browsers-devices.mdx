...
You can find our supported range of browsers and their versions [in our `.browserslistrc file`]([[config:repo]]/blob/v[[config:current_version]]-ouds-web/.browserslistrc):

<Code lang="plaintext" filePath=".browserslistrc" />

<Callout type="info">
  We strongly suggest using our <code>.browserlistrc</code> in projects using OUDS SCSS. This will avoid any potential issues that might arise because of browsers specificity.<br/><br/>
  For most projects, you will simply need to add the file to the root of the repository, some might need a more complex config.
</Callout>
