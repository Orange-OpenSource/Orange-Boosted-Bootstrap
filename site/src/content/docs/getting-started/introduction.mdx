...
Get started by including OUDS Web’s production-ready CSS and JavaScript via CDN without the need for any build steps. See it in practice with this [OUDS Web CodePen demo]([[config:debug_template]]).
...
When using CDN links, be sure to use the `integrity` attribute to verify the correct files and versions. These hashes are unique to each file and version of OUDS Web, so when you update to a new version, be sure the `integrity` attribute is also updated.
...
- Read about what’s included in OUDS Web in our [contents section]([[docsref:/getting-started/contents]]).
- Read about browser support in our [Browsers and devices section]([[docsref:/getting-started/browsers-devices]]).
{/* and the list of [components that require JavaScript](#js-components) below */}
...
## Using OUDS Web with AI coding agents

OUDS Web ships [Agent Skills](https://www.skills.sh/) in the [`skills/` directory](https://github.com/Orange-OpenSource/Orange-Boosted-Bootstrap/tree/main/skills) of the repository. These give AI coding agents (Claude Code, Cursor, GitHub Copilot, and others) procedural knowledge of OUDS Web, such as correct component markup, layout and utility classes, design tokens, and migration steps from Boosted, OB1, or older OUDS Web versions.
...
Here is a list of components that explicitly require our JavaScript and Popper for interactivity.

{/* - Accordions for extending our Collapse plugin */}

- [Alerts]([[docsref:/components/alerts#javascript]]) for dismissing {/*and Close button tooltip*/}
- [Chips]([[docsref:/components/chips#javascript]]) for toggling states
...
For improved cross-browser rendering, we use [Reboot]([[docsref:/foundation/reboot]]) to correct inconsistencies across browsers and devices while providing slightly more opinionated resets to common HTML elements.
