...
<Callout type="warning">
  ...
  * Change `bootstrap` to `ouds-${getConfig().brand}` in `scss/style.scss`.
  * Replace `bootstrap` with `@ouds/web-common` and `@ouds/web-${getConfig().brand}` with the correct versions in `package.json`.
