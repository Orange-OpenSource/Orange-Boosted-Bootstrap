...
<Callout type="info" title="Try it yourself!">
  Download the source code and working demo for using Bootstrap as an ES module from the [twbs/examples repository](https://github.com/twbs/examples/tree/main/sass-js-esm). You can also [open the example in StackBlitz](https://stackblitz.com/github/twbs/examples/tree/main/sass-js-esm?file=index.html).
</Callout>
...
<Callout type="warning">
  ...
  * Change `bootstrap` to <code>ouds-web-{getConfig().brand}</code> in `scss/style.scss`.
  * Replace `bootstrap` with `@ouds/web-common` and <code>@ouds/web-{getConfig().brand}</code> with the correct versions in `package.json`.
  * Replace CDN URLs with the correct ones within `import` statements in `index.html` (see [CND links]([[docsref:/getting-started/introduction#cdn-links]])).
...
<Code buttonLabel="OUDS Web as a module" lang="html" code={`
  <script type="module">
...
<Code buttonLabel="import Popper" lang="js" code={`import * as Popper from "@popperjs/core"`} />

If you try this as-is, you'll see an error in the console like the following:

<Code buttonLabel="error" lang="text" code={`Uncaught TypeError: Failed to resolve module specifier "@popperjs/core". Relative references must start with either "/", "./", or "../".`} />

To fix this, you can use an `importmap` to resolve the arbitrary module names to complete paths. If your [targeted browsers](https://caniuse.com/?search=importmap) do not support `importmap`, you'll need to use the [es-module-shims](https://github.com/guybedford/es-module-shims) project. Here's how it works for OUDS Web and Popper:

<Code buttonLabel="importmap" lang="html" code={`<!doctype html>
  <html lang="en">
    <head>
      <meta charset="utf-8">
    ...
    <body>
      ...
      <script type="importmap">
      {
        "imports": {
          ...
          "@ouds/web-common": "https://cdn.jsdelivr.net/npm/@ouds/web-common@{{< param "current_version" >}}/dist/js/ouds-web.esm.min.js"
        }
      }
      </script>
      <script type="module">
...
<Code buttonLabel="events" lang="js" code={`const myModal = document.querySelector('#myModal')
...
<Code buttonLabel="programmatic API" lang="js" code={`const myModalEl = document.querySelector('#myModal')
...
<Code buttonLabel="getInstance" lang="js" code={`oudsWeb.Popover.getInstance(myPopoverEl)`} />
...
<Code buttonLabel="getOrCreateInstance" lang="js" code={`oudsWeb.Popover.getOrCreateInstance(myPopoverEl, configObject)`} />
...
<Code buttonLabel="constructor, getInstance and getOrCreateInstance" lang="js" code={`const modal = new oudsWeb.Modal('#myModal')
...
<Code buttonLabel="listen to event" lang="js" code={`const myCollapseEl = document.querySelector('#myCollapse')
...
<Code buttonLabel="asynchronous functioning" lang="js" code={`const myCarouselEl = document.querySelector('#myCarousel')
...
<Code buttonLabel="`dispose` method" lang="js" code={`const myModal = document.querySelector('#myModal')
...
<Code buttonLabel="change settings" lang="js" code={`// changes default for the modal plugin's \`keyboard\` option to false
...
<Code buttonLabel="" lang="js" code={`const myDefaultAllowList = oudsWeb.Tooltip.Default.allowList
...
<Code buttonLabel="" lang="js" code={`const yourTooltipEl = document.querySelector('#yourTooltip')
...
<Code buttonLabel="" lang="js" code={`// to enable tooltips with the default configuration
...
<Code buttonLabel="" lang="js" code={`const oudsWebButton = $.fn.button.noConflict() // return $.fn.button to previously assigned value
...
<Code buttonLabel="" lang="js" code={`$('#myTab a').on('shown.bs.tab', () => {
    // do something...
  })`} />
