...
<Code buttonLabel="events" lang="js" code={`const myDialog = document.querySelector('#myModal')
...
<Code buttonLabel="programmatic API" lang="js" code={`const myDialog = document.querySelector('#myModal')
...
<Code buttonLabel="constructor, getInstance and getOrCreateInstance" lang="js" code={`const dropdown = new oudsWeb.Dropdown('[data-bs-toggle="dropdown"]')
...
<Code buttonLabel="change settings" lang="js" code={`// Dialog behavior is configured in markup and native dialog APIs
...
<BsTable class="table">
  | Method | Description |
  | --- | --- |
  | `showModal` | Native dialog API to open a modal dialog. |
  | `close` | Native dialog API to close a dialog. |
  | `requestClose` | Native dialog API to request close with cancel support (where available). |
</BsTable>
