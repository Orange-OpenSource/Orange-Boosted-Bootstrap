...
<Callout type="info" title="Draft components">
  Some components are marked as draft because they have not been designed yet. They are available in OUDS Web to speed up development but are subject to change in the future.
</Callout>
...
<table class="table">
  ...
  {
    componentsDetails
      .filter((component) => !component.excludeFromVersioning)
        ...
        <tr>
          <td>
          ...
          </td>
            {
              component.version ?
                <td>
                  <span class="tag tag-info tag-muted">{component.version.value}</span>&nbsp;
                  <a href={`https://r.orange.fr/r/S-ouds-doc-${component.slug ?? component.name.toLowerCase()}_changelog`}>
            changelog<span class="visually-hidden">for {component.name}</span></a>
                </td> :
                <td><span class="tag tag-warning tag-muted">draft</span></td>
            }
        </tr>
      ))
  }
