...
Color and background helpers combine the power of our [`.text-*` utilities]([[docsref:/utilities/color]]) and [`.bg-*` utilities]([[docsref:/utilities/background]]) in one class. We determine a contrasting `color` for a particular `background-color`. However, it doesn’t change mode at all.

<Callout type="info">
Please consider using our [color modes]([[docsref:/foundation/color-modes]]) directly as explained in our [background utilities]([[docsref:/utilities/background]]) to set nested components in the right mode directly as well.
</Callout>
