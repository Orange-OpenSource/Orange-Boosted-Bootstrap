...
import { allTokens, scssMaps } from '@libs/sass-variables'
...
<Example code={[
  ...scssMaps.find(token => token.name === '$ouds-backgrounds').mapValue
    .filter(bgToken => !['transparent', 'opacity-lowest', 'opacity-lower', 'always-black', 'always-white'].includes(bgToken.name))
    .map(token => {
      const conditionalBg = token.value.match(/variable-exists\(([^\)]*)/g)
      // remove backgrounds that are conditional
      if(conditionalBg && !allTokens.includes(conditionalBg[0]))  return '';
