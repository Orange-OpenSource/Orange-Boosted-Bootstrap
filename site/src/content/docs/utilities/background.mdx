...
*Depending on the context of the element with a `.bg-*` class and its resulting color, you should add a unique child element with a `[data-bs-theme]` [attribute to specify the color mode]([[docsref:/foundation/color-modes#how-to-use]]) for its descendants.*
...
*Note:* It is not possible to add the attribute directly on the `.bg-*` element, as the background color would then adapt to the attribute instead of conforming to the color mode set by its ancestors.
