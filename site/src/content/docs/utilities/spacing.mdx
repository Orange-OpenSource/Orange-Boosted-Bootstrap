...
<ul>
  {
    fixedSpacingLevel.map((level) => (
      <li><code>{level}</code> - for classes that set
        the <code>margin</code> or <code>padding</code> to <code>{allTokens.find((token) => token.name === `$ouds-space-fixed-${level}`).compiledValue}</code>
      </li>
    ))
  }
      ...
      scaledSpacingLevel.map((level) => (
        <tr>
          ...
          </td>
          {
            screenSize.map((size) => (
              <td>
                <code>{allTokens.find((token) => token.name === `$ouds-space-scaled-${level}-${size}`).compiledValue}</code>
              </td>
            ))
          }
        </tr>
...
<Example class="bd-example-spacings" code={`<div class="px-gridgap">Gridgap padding</div>
  <div class="px-xl-gridgap">Grid-gap padding for xl viewports</div>
