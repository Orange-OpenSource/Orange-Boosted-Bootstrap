...
<ul>
  ...
  <li>`gridgap` - for classes that set the `margin` to `var(--bs-grid-gap)` [see below](#grid-specific-spacings)</li>
  <li>`gridmargin` - for classes that set the `margin` to `var(--bs-grid-margin)` [see below](#grid-specific-spacings)</li>
</ul>
...
OUDS web provides two specific spacings based on grid measures: `gridgap` and `gridmargin` which use the custom properties `--bs-grid-gap` and `--bs-grid-margin`, respectively. These spacings can be applied as `margin`, `padding` and `gap` utilities and are responsive.
...
The negative `gridmargin` `margin` utility can be used to create edge to edge containers (this example creates an edge to edge container on the smaller breakpoints).

<div class="mx-ngridmargin mx-md-auto py-small px-md-small px-gridmargin mb-small" style="background-color: rgba(var(--bd-primary-light-rgb), 0.15);">Edge to edge container on smaller breakpoints</div>

```html
<div class="mx-ngridmargin mx-md-auto py-small px-md-small px-gridmargin">
