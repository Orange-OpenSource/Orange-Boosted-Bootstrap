...
Content color utility classes, in the form  `.text-*`, apply a color [token]([[docsref:/foundation/tokens]]) as `color` for a [content]([[docsref:/foundation/colors#background]]) or ['always' color]([[docsref:/foundation/colors#always]]) dynamically depending on the [color mode]([[docsref:/foundation/color-modes]]).
