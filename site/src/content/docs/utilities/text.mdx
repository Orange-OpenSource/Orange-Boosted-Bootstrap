...
<Callout type="info">
  Center and end alignment should only be used in specific cases. When you must use them, combine `.{breakpoint}:text-center` with `.{breakpoint}:mx-auto` for centering or `.{breakpoint}:text-end` with `.{breakpoint}:ms-auto` for end alignment.
</Callout>
