...
We encourage you to take the JavaScript from `@ouds/web-common` for a smoother integration with OUDS Web. So replace any import like `boosted/dist/js/boosted.bundle.min.js` by `@ouds/web-common/dist/js/ouds-web.bundle.min.js`.

However, please note that OUDS Web no longer maintain jQuery compatibility. If your project still depends on jQuery plugins or requires jQuery compatibility, you should continue using Boosted's JavaScript (`boosted/dist/js/boosted.bundle.min.js`) until you can eliminate the jQuery dependency.
...
##### With granular Boosted imports

<Callout type="info">
  **This is the recommended scenario.**

  Note that you won’t be able to:
  - use any Sass variable or mixin from OUDS Web in this scenario. Only the one from Boosted will be available.

  If you want to have both, you may separate the 2 builds into different files and import the result in the main style file of your project. It will allow you to tree shake OUDS Web as well.
</Callout>
...
2. Replace your Boosted import file by copy pasting this code in the main style file of your project:
    ...
    // Please note that if you don’t need any Boosted variables in your custom components,
    // all the Configuration imports may be scoped inside the .boosted class to avoid side effects
    ...
    .boosted {
      ...
      // It will avoid having most edge cases and unwanted side effects of OUDS Web inside Boosted.
      // It will also allow you to use the dark mode of OUDS Web inside Boosted.
      [data-bs-theme] {
      ...
      :root[data-bs-theme="dark"] &[data-bs-theme="root-inverted"] {
        ...
        --#{$prefix}modal-border-radius: 0px;
...
3. Once you’re all set, you can begin to follow [our migration guide from Boosted]([[docsref:/getting-started/migration-from-boosted]]).
...
##### With full Boosted imports

<Callout type="warning">
  This is not the recommended scenario, but if you want a simpler migration (at first only), here is a way to do it.

  You won’t be able to:
  - use any Sass variable or mixin from OUDS Web in this scenario.
  - tree shake Boosted.
  - change easily the edge cases and unwanted side effects of OUDS Web.
  - use dynamic modes in Boosted.
</Callout>
...
2. Replace your Boosted import file by copy pasting this code in the main style file of your project:
    ...
    // 2. Import all Boosted
    .boosted {
      $root-selector: '&';
      @import "boosted/scss/boosted";
    }

    // 3. Import custom components from your project with possible definitions of the variables you need.
    @import "your/custom/(s)css/files.scss";`} />
    </details>

3. Once you’re all set, you can begin to follow [our migration guide from Boosted]([[docsref:/getting-started/migration-from-boosted]]), and add a `.boosted` each time you need to use Boosted on the element or its children and also to set the `[data-bs-theme]` every time you use this selector. Don’t change any element further since you’ll be using Boosted directly.
...
<Callout type="warning">
  ...
  You won’t be able to:
  - use any Sass variable or mixin from Boosted in this scenario.
  - use OUDS Web containers, grid and utilities.
  - use dynamic modes in Boosted.
</Callout>
...
2. Replace your Boosted import file by copy pasting this code in the main style file of your project:
    <details class="mb-medium">
      <summary>See the main style file example</summary>
      <Code lang="scss" code={`// 1. Import all Boosted or make your own build in a different file to avoid variables conflicts
    ...
    .ouds-web {
      ...
      @import "@ouds/web-common/scss/table"; // Draft and might change
      @import "@ouds/web-common/scss/forms/control-item";
      @import "@ouds/web-common/scss/forms/select-input";
      @import "@ouds/web-common/scss/forms/text-input";
      @import "@ouds/web-common/scss/links";
      @import "@ouds/web-common/scss/bullet-list";
      @import "@ouds/web-common/scss/buttons";
      @import "@ouds/web-common/scss/button-assistant";
      @import "@ouds/web-common/scss/chips";
      @import "@ouds/web-common/scss/dropdown"; // Draft and might change
      @import "@ouds/web-common/scss/nav"; // Draft and might change
      @import "@ouds/web-common/scss/navbar"; // Draft and might change
      @import "@ouds/web-common/scss/breadcrumb";
      @import "@ouds/web-common/scss/badges";
      @import "@ouds/web-common/scss/tags";
      @import "@ouds/web-common/scss/alert";
      @import "@ouds/web-common/scss/items";
      @import "@ouds/web-common/scss/skeleton";
      @import "@ouds/web-common/scss/footer"; // Draft and might change
      @import "@ouds/web-common/scss/orange-navbar"; // Draft and might change
...
3. Once you’re all set, you can begin to follow [our migration guide from Boosted]([[docsref:/getting-started/migration-from-boosted]]), and add a `.ouds-web` each time you need to use OUDS Web on the element or its children and also to set the `[data-bs-theme]` every time you use this selector.
