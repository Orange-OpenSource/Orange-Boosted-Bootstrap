...
<Example buttonLabel="colored horizontal rules" class="d-flex flex-column gap-1" code={[...allTokens.filter(token => token.name.match(/\$ouds-color-border-/) && token.name.match(/-light/) && token.compiledValue && !token.name.match(/-focus/)).map(token => {
    if (token.name.match(/-on-/)) {
      return `<div class="bg-${token.name.split('-').slice(4, -1).join('-')} py-2xs"><hr class="border-${token.name.split('-').slice(3, -1).join('-')}"></div>`
    }
...
<Example buttonLabel="colored vertical rules" class="d-flex flex-column gap-1" code={[...allTokens.filter(token => token.name.match(/\$ouds-color-border-/) && token.name.match(/-light/) && token.compiledValue && !token.name.match(/-focus/)).map(token => {
    if (token.name.match(/-on-/)) {
      return `<div class="bg-${token.name.split('-').slice(4, -1).join('-')} py-2xs"><div class="vr border-${token.name.split('-').slice(3, -1).join('-')}"></div></div>`
    }
