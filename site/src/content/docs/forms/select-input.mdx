...
An empty placeholder `<option>` is required on each `<select>` as our CSS-only floating label implementation relies on it to detect when the select is empty. This one should be disabled so users won't be able to select it. An empty value to avoid displaying an option as placeholder.
