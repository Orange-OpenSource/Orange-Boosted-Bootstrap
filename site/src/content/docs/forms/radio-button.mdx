...
It is best practice to provide clear context by adding a `<legend>` element inside the `<fieldset>` for the group title. Then, screen readers will read the legend before navigating through the radio buttons.
