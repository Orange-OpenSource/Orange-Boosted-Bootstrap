...
As radio buttons are almost always grouped in a list, they should be enclosed in a `<fieldset> ` element with a `.control-items-list` class; this will handle automatic error text display when one of the control items input is invalid.
