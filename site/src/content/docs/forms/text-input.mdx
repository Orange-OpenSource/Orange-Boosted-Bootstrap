...
You must have the svg already in the `.text-input-container` to make it work like charm. You may also have a status span to display the loading message on assistive technologies.
...
<Callout type="info">
  Optionally, the input can be disabled or not during the loading time depending on the context of use.
</Callout>
