...
{getData('palette').filter(palette => palette.category === "ODS colors").map((category) => {
  return(
          ...
          return (
            <figure class="mb-0" aria-label={color.name}>
              <button class="btn border-0 p-0 color-copy ratio ratio-1x1" data-clipboard-text={color.variable} data-bs-toggle="tooltip" data-bs-title="Copy to clipboard">
                <svg viewBox="0 0 100 100" aria-hidden="true" preserveAspectRatio="xMidYMid meet" style={{border: (color.name === "White 100" || color.name === "Black 900") ? "1px solid var(--bs-border-color-subtle)" : ""}}>
              ...
              <figcaption class="py-1">
                ...
                <hr class="my-1 bg-transparent border-top" className="" style={`border-color: ${color.hex} !important`} />
...
{getData('colors').map((color) => {
  if (color.name !== "white" && color.name !== "gray" && color.name !== "gray-dark") {
    return (
      <div class="col-md-4 mb-3" key={color.name}>
        <div class={`p-3 mb-2 position-relative swatch-${color.name}`}>
          <strong class="d-block">${color.name}</strong>
          {color.hex}
        </div>
        {[100,200,300,400,500,600,700,800,900].map((level) => (
          <div class={`p-3 bd-${color.name}-${level}`} key={level}>
            ${color.name}-{level}
          </div>
        ))}
      </div>
    ...
    return (
      <div class="col-md-4 mb-3" key={color.name}>
        ...
        {[100,200,300,400,500,600,700,800,900,950].map((level) => (
          <div class={`p-3 bd-${color.name}-${level}`} key={level}>
            ${color.name}-{level}
          </div>
        ))}
...
<ScssDocs name="palette" file="scss/_color-palette.scss" />
