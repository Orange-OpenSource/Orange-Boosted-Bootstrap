...
<Code lang="css" filePath="dist/css/boosted.css" fileMatch=":(root,\n\[data-bs-theme=light\] {[^}]*})" />

### Dark mode

These variables are scoped to our built-in dark mode.

<Code lang="css" filePath="dist/css/boosted.css" fileMatch="(\[data-bs-theme=dark\] {[^}]*})" />
...
Boosted uses [embedded SVGs as data URIs]([[docsref:/customize/overviews#csps-and-embedded-svgs]]) in the wild, which means extremely long strings in CSS. When one of them is used several times in the stylesheet, CSS custom properties allows to factorize its string— thus to decrease output file size.
