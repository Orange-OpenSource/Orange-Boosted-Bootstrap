...
All heading elements—`<h1>`—`<h6>` have their `margin-top` removed, <code>margin-bottom: {allTokens.find((token) => token.name === '$ouds-space-fixed-md').compiledValue}</code> set, and `line-height` tightened. While headings inherit their `color` by default, you can also override it via optional CSS variable, `--bs-heading-color`.
...
All `<p>` elements have their `margin-top` removed and <code>margin-bottom: {allTokens.find((token) => token.name === '$ouds-space-fixed-md').compiledValue}</code> set for easy spacing. If you want to get rid of the `margin-bottom`, please make sure to use our `.mb-none` [spacing utilities]([[docsref:/utilities/spacing]]).
