...
export const toto = allTokens.filter((token) => token.name.match(new RegExp(`^\\$core-(ouds|${getConfig().brand})-color`))).reduce((accColors, color) => {
...
{/* Please make sure that none of the [color theme variables]([[docsref:/foundation/color-theme]]) fit your needs before picking one here. */}
        ...
        <figure class="mb-none" aria-label={color.name}>
          <button class="border-none p-none color-copy ratio ratio-1x1" data-clipboard-text={color.name} data-bs-toggle="tooltip" data-bs-title="Copy to clipboard">
            <svg viewBox="0 0 100 100" class="rounded" aria-hidden="true" preserveAspectRatio="xMidYMid meet">
              ...
              {color.compiledValue.includes('rgba') && (
                <>
                  <svg class="decorative-xsmall-icon text-always-black" x="15" y="15" width="20" height="20" aria-hidden="true">
                    <use xlink:href={getVersionedDocsPath('/assets/img/ouds-web-sprite.svg#ui-light-mode')}/>
                  </svg>
                  <svg class="decorative-xsmall-icon text-always-white" x="65" y="15" width="20" height="20" aria-hidden="true">
          ...
          <figcaption class="py-2xsmall">
            ...
            <hr class="my-2xsmall bg-transparent border-top" style={`border-color: ${color.compiledValue} !important`} />
...
{/* , you can find most of them in our [color theme section]([[docsref:/foundation/color-theme]]) */}
...
<div class="row font-monospace">
  {getData('colors').map((color) => color.name !== 'white' && color.name !== 'gray' && color.name !== 'gray-dark' && (
    <div class="col-md-4 mb-large">
      <div class={`p-large mb-small position-relative swatch-${color.name}`}>
        <strong class="d-block">{color.name}</strong>
        {color.hex}
      </div>
      {[...Array(9).keys()].map((value) => {
        return (
          <div class={`p-large bd-${color.name}-${value * 100 + 100}`}>{color.name}-{value * 100 + 100}</div>
        )
      })}
    </div>
  ))}
  ...
  <div class="col-md-4 mb-large">
    <div class="p-large mb-small position-relative swatch-gray-500">
      <strong class="d-block">$gray-500</strong>
      #555
    </div>
    {getData('grays').map((gray) => {
      return (
        <div class={`p-large bd-gray-${gray.name}`}>$gray-{gray.name}</div>
      )
    })}
  </div>

  <div class="col-md-4 mb-large">
    <div class="p-large mb-small bd-black text-always-on-black">
      <strong class="d-block">$black</strong>
      #000
    </div>
    <div class="p-large mb-small bd-white border">
...
[Color]([[docsref:/utilities/color]]) and [Background]([[docsref:/utilities/background]]) utility classes are also available for setting `color` and `background-color`.
</BootstrapCompatibility>
