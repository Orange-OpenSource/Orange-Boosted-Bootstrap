...
export const palette = allTokens.filter((token) => token.name.match(new RegExp(`^\\$core-(ouds|${getConfig().brand})-color`))).reduce((accColors, color) => {
...

{
  Object.keys(palette).map((colorCategory) => (
    <div class="row row-cols-1 sm:row-cols-2 md:row-cols-4 lg:row-cols-3 xl:row-cols-5 gy-2xlarge pb-large">
      {palette[colorCategory].map((color) => (
        <figure class="mb-none" aria-label={color.name}>
          <button class="border-none p-none color-copy w-100 ratio-1x1" data-clipboard-text={color.name} data-bs-toggle="tooltip" data-bs-title="Copy to clipboard">
