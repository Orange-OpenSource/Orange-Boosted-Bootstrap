...
Alternatively you can programmatically add a `aria-invalid="true"` to manually mark a field as invalid, this is especially useful for [server-side validation](#server-side).
...
- As a fallback, `aria-invalid="true"` attribute may be used instead of the pseudo-class `:user-invalid` for [server-side validation](#server-side).
...
The following script is just a stub doing the minimum to handle inputs description and ARIA description when fields are invalid. We are using a data attribute to store the descriptions ids and change the `aria-describedby` attribute when necessary.
