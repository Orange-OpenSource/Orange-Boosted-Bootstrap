...
In a form having several fields and where not all fields are mandatory, you have to display a message "**All fields marked with an <span class="text-status-negative">*</span> are mandatory.**" at the top, and hide it from assistive technologies with `aria-hidden="true"`, since the mandatory nature of the fields will be indicated to assistive technlogies by the `required` attribute. Then, you have to explicitly indicate which fields are mandatory to other users, by adding the class `.is-required` on the label, which adds a red asterisk at the end of the label.
