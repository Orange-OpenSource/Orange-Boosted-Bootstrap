...
<Callout type="info">
  To have the asterisk in the right color for disabled fields, you may disable a `<fieldset>` with `disabled`, or add `aria-disabled="true"` on the fieldset/container (or on the label).
</Callout>
