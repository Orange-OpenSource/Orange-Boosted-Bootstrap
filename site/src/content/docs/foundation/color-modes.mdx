...
<Callout type="warning">
  Be careful when using the `data-bs-theme` attribute on an element carrying one of our [`.bg-*` utilities]([[docsref:/utilities/background]]), the `background-color` property will also be automatically changed to use the current defined color mode. **Use it on two different elements.**
</Callout>
...
You should apply a `[data-bs-theme]` attribute whenever **you need to change the text color** and the component modes inside.
...
<Example class="d-flex justify-content-between flex-wrap gap-2xsmall" code={`<div class="dropdown" data-bs-theme="light">
...
{/* if you like in our [Orange navbar section]([[docsref:/components/orange-navbar#with-mode-selector]]) */}
