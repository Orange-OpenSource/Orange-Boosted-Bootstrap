...
Use the `data-bs-theme` attribute to set the color mode on a container. It can be applied on the `<html>` element to support the global [color mode toggle](#javascript). By default, our CSS behaves as if `data-bs-theme="light"` were set on `<html>`, even when the attribute is not present.
...
We offer two static color modes: `"light"` and `"dark"`. These modes are called static because once the theme is set on an element, it remains fixed regardless of the root theme. The `"light"` theme will set the text color to black and the descendant components will adapt their colors accordingly. Conversely, the `"dark"` theme will set the text color to white, with descendant components adapting their colors to match.
...
We offer two dynamic modes: `"root"` and `"root-inverted"`. These modes are called dynamic because they depend on the root element mode, so by changing the main mode, you’re changing all the dynamic areas using these modes as well. The `"root"` theme will reset the color mode to the root mode of your page, while the `"root-inverted"` mode will set the color mode to the opposite theme of your page.

<Callout type="info">
  By definition these modes **should not be used on the root element**.
</Callout>
...
This color mode inverts the main color mode. It will be useful for example inside a container with a darker background in root light mode like `.bg-inverse-high`. To apply this mode add <span class="user-select-all">`data-bs-theme="root-inverted"`</span> on any element but the main one.
