...
All heading elements—`<h1>`—`<h6>` have their `margin-top` removed, <code>margin-bottom: {allTokens.find((token) => token.name === '$ouds-space-fixed-medium').compiledValue}</code> set, and `line-height` tightened. While headings inherit their `color` by default, you can also override it via optional CSS variable, `--bs-heading-color`.
...
All `<p>` elements have their `margin-top` removed and <code>margin-bottom: {allTokens.find((token) => token.name === '$ouds-space-fixed-medium').compiledValue}</code> set for easy spacing. If you want to get rid of the `margin-bottom`, please make sure to use our `.mb-none` [spacing utilities]([[docsref:/utilities/spacing]]).
...
<Example buttonLabel="links on colored backgrounds" class="p-none" code={`<div class="colored-bg">
...
The `<address>` element is updated to reset the browser default `font-style` from `italic` to `normal`. `line-height` is also now inherited, and <code>margin-bottom: {allTokens.find((token) => token.name === '$ouds-space-fixed-medium').compiledValue}</code> has been added. `<address>`s are for presenting contact information for the nearest ancestor (or an entire body of work). Preserve formatting by ending lines with `<br />`.
...
The default `margin` on blockquotes is `1em 40px`, so we reset that to <code>0 0 {allTokens.find((token) => token.name === '$ouds-space-fixed-medium').compiledValue}</code> for something more consistent with other elements.
