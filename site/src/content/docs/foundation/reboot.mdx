...
<Code lang="html" buttonLabel="real-time customization" code={`<body style="--bs-body-color: #333;">
    <!-- ... -->
  </body> `} />
...
All heading elements—`<h1>`—`<h6>` have their `margin-top` removed, <code>margin-bottom: {getTokenValue('$ouds-space-fixed-medium')}</code> set, and `line-height` tightened. While headings inherit their `color` by default, you can also override it via optional CSS variable, `--bs-heading-color`.

<BsTable class="table">
| Heading | Example |
| --- | --- |
| `<h1></h1>` | <p class="h1">h1. OUDS Web heading</p> |
| `<h2></h2>` | <p class="h2">h2. OUDS Web heading</p> |
| `<h3></h3>` | <p class="h3">h3. OUDS Web heading</p> |
| `<h4></h4>` | <p class="h4">h4. OUDS Web heading</p> |
| `<h5></h5>` | <p class="h5">h5. OUDS Web heading</p> |
| `<h6></h6>` | <p class="h6">h6. OUDS Web heading</p> |
</BsTable>

## Paragraphs

All `<p>` elements have their `margin-top` removed and <code>margin-bottom: {getTokenValue('$ouds-space-fixed-medium')}</code> set for easy spacing. If you want to get rid of the `margin-bottom`, please make sure to use our `.mb-none` [spacing utilities]([[docsref:/utilities/spacing]]).

<Example buttonLabel="paragraph" code={`<p class="mw-none mb-none">This is an example paragraph which is long enough so it’s easy to see and compare the <code>max-width</code> in action and compare it to the one below.</p>
...
<Example buttonLabel="link" code={`<a href="#">This is an example link</a>`} />

Links have a disabled style when using an `aria-disabled="true"` attribute.

<Example buttonLabel="disabled link" code={`<a aria-disabled="true">This is an example of a disabled link</a>`} />

Visited links don’t have a specific style by default.

<ThemeSpecific theme={['orange', 'orange-compact']}>
The `:visited` state can be styled with the additional `.visited-links` utility class that can be applied directly to a link or to a parent element to style all the child links.

<Example buttonLabel="visited link" code={`<div class="visited-links">
    <div>
...
<Example buttonLabel="placeholder link" code={`<a>This is a placeholder link</a>`} />
...
<Example buttonLabel="links on colored backgrounds" class="p-none" code={`<div class="link-on-colored-bg">
  ...
  <div class="link-on-colored-bg bg-surface-brand-primary p-large">
    <div data-bs-theme="${getMode('modes-on-brand-primary')}">
      This is an example of a <a href="." class="visited-links">link on colored background</a> in a paragraph.
    </div>
  </div>`} />
...
The `<hr />` style can be modified with border utilities. See some customization examples in the [Divider helpers]([[docsref:/components/divider]]) documentation.
...
<Example buttonLabel="description list" showMarkup={false} code={`<dl>
...
<Example buttonLabel="inline code" code={`For example, <code>&lt;section&gt;</code> should be wrapped as inline.`} />
...
<Example buttonLabel="code block" code={`<pre><code>&lt;p&gt;Sample text here...&lt;/p&gt;
...
<Example buttonLabel="variable" code={`<var>y</var> = <var>m</var><var>x</var> + <var>b</var>`} />

## User input

Use the `<kbd>` to indicate input that is typically entered via keyboard.

<Example buttonLabel="user input" code={`To switch directories, type <kbd>cd</kbd> followed by the name of the directory.<br />
...
<Example buttonLabel="sample output" code={`<samp>This text is meant to be treated as sample output from a computer program.</samp>`} />

## Tables

Tables are slightly adjusted to style `<caption>`s, collapse borders, and ensure consistent `text-align` throughout. Additional changes for borders, padding, and more come with [the `.table` class]([[docsref:/components/table]]).

<Example buttonLabel="table" code={`<table>
    <caption>
...
<Callout type="warning" name="warning-input-support" />

<Example buttonLabel="form" showMarkup={false} code={`<form>
    <fieldset>
...
<Example buttonLabel="pointer on button" code={`<span role="button" tabindex="0">Non-button element button</span>`} />

## Misc elements

### Address

The `<address>` element is updated to reset the browser default `font-style` from `italic` to `normal`. `line-height` is also now inherited, and <code>margin-bottom: {getTokenValue('$ouds-space-fixed-medium')}</code> has been added. `<address>`s are for presenting contact information for the nearest ancestor (or an entire body of work). Preserve formatting by ending lines with `<br />`.

<Example buttonLabel="address" showMarkup={false} code={`<address>
...
The default `margin` on blockquotes is `1em 40px`, so we reset that to <code>0 0 {getTokenValue('$ouds-space-fixed-medium')}</code> for something more consistent with other elements.

<Example buttonLabel="blockquote" showMarkup={false} code={`<figure>
...
<Example buttonLabel="abbreviation" showMarkup={false} code={`The <abbr title="HyperText Markup Language">HTML</abbr> abbreviation element.`} />

### Summary

The default `cursor` on summary is `text`, so we reset that to `pointer` to convey that the element can be interacted with by clicking on it.

<Example buttonLabel="summary" showMarkup={false} code={`<details>
