...
All heading elements—`<h1>`—`<h6>` have their `margin-top` removed, `margin-bottom` set, and `line-height` tightened. While headings inherit their `color` by default, you can also override it via optional CSS variable, `--bs-heading-color`.
...
All `<p>` elements have their `margin-top` removed and `margin-bottom` set for easy spacing. If you want to get rid of the `margin-bottom`, please make sure to use our `.mb-none` [spacing utilities]([[docsref:/utilities/spacing]]).
...
The default `margin` on blockquotes is `1em 40px`, so we reset that for something more consistent with other elements.
