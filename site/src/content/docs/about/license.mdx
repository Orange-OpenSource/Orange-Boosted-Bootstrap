...
OUDS Web is released under the MIT license and is copyright {new Date().getFullYear()} Orange SA for additions to native Bootstrap, which is released under MIT license and copyright {new Date().getFullYear()}.
...
For more information, the [full OUDS Web license is located in the project repository]([[config:repo]]/blob/v[[config:current_version]]-ouds-web/LICENSE), and [Bootstrap license is located in the project repository](https://github.com/twbs/bootstrap/blob/v[[config:current_version]]/LICENSE).
