...
export function getVersionedBsCssProps(direction: 'rtl' | undefined) {
  let bsCssLinkHref = '/dist/css/boosted'
...
export function getVersionedBsFontProps(direction: 'rtl' | undefined) {
  let bsCssLinkHref = '/dist/css/orange-helvetica'
  ...
  if (import.meta.env.PROD) {
    bsCssLinkProps.integrity = direction === 'rtl' ? getConfig().cdn.helvetica_rtl_hash : getConfig().cdn.helvetica_hash
  }
...
export function getVersionedBsJsProps() {
  let bsJsScriptSrc = '/dist/js/boosted.bundle'
