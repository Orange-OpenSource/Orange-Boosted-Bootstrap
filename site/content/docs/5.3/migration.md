...
- **Typography**
  ...
  - <span class="badge bg-warning">Breaking</span> Some headings font size values have been changed to be compliant with the design guidelines. Please check it has no impact on your websites.
    - `<h3>` values are now `24px` for desktop and tablet breakpoints and `18px` for mobile breakpoint.
    - `<h5>` and `<h6>` values are now `18px` for desktop and tablet breakpoints and `16px` for mobile breakpoint.
