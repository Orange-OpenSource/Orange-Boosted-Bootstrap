...
description: Make any HTML element or Boosted Innovation Cup component clickable by "stretching" a nested link via CSS.
group: helpers
aliases:
...
Cards have `position: relative` by default in Boosted Innovation Cup, so in this case you can safely add the `.stretched-link` class to a link in the card without any other HTML changes.
