...
Limit the line length with `.ll-*` utilities. It applies `max-width` rule to an element with a width based on `ch` dimension. It only applies on [block-level elements](https://developer.mozilla.org/fr/docs/Web/HTML/Block-level_elements) and not on [inline elements](https://developer.mozilla.org/fr/docs/Web/HTML/Inline_elements).
