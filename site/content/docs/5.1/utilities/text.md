...
Limit the line length with `.ll-*` utilities. It applies `max-width` property to an element with a width based on `ch` unit. We chose this one because it is a font-based unit and because 1`ch` is the width of '0' (which is around an average caracter width). It requires anything but `inline` to be applied.
