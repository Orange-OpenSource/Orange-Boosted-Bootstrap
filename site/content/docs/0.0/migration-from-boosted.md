...
- <span class="badge text-bg-warning">Warning</span> You might be able to transfer your Spacing utilities following one of the two proposed methods (while keeping almost the rendering and at least the same proportion between spacings):
  - Keep the previous behavior and getting one utility for another.
    1. **0**: `(([mp]|gap)\S?-\S?\S?\S?-?n)?0` → `$1none`
    2. **1**: `(([mp]|gap)\S?-\S?\S?\S?-?n?)1` → `$1shortest`
    3. **2**: `(([mp]|gap)\S?-\S?\S?\S?-?n?)2` → `$1shorter`
    4. **3**: `(([mp]|gap)\S?-\S?\S?\S?-?n?)3` → `$1medium`
    5. **4**: `(([mp]|gap)\S?-\S?\S?\S?-?n?)4` → `$1tall`
    6. **5**: `(([mp]|gap)\S?-\S?\S?\S?-?n?)5` → `$1spacious`
    7. **All numbers**: Search for `([mp]|gap)\S?-\S?\S?\S?-?n?\d` and you shouldn't have Bootstrap Spacing utilities anymore
  - Change all the utilities using more precise utilities
    1. **Mixing Bootstrap numbers**: Search for `([mp]|gap)\S?-\S?\S?\S?-?n?\d`, when you match multiple on one line, consider using one of our [scaled spacing utilities]({{< docsref "/utilities/spacing#notation-for-scaled-values" >}}) (for example, `.mb-1.mb-md-2` → `.mb-scaled-shortest`).
