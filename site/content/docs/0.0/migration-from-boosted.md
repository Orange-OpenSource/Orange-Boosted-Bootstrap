...
- <span class="badge text-bg-warning">Warning</span> You might be able to transfer your Spacing utilities following one of the two proposed methods (while keeping almost the rendering and at least the same proportion between spacings):
  - Keep the previous behavior and getting one utility for another.
    1. **0**: `([mp]\S?-?\S?\S?\S?)-0` → `$1-none`
    2. **1**: `([mp]\S?-?\S?\S?\S?)-1` → `$1-shortest`
    3. **2**: `([mp]\S?-?\S?\S?\S?)-2` → `$1-shorter`
    4. **3**: `([mp]\S?-?\S?\S?\S?)-3` → `$1-medium`
    5. **4**: `([mp]\S?-?\S?\S?\S?)-4` → `$1-tall`
    6. **5**: `([mp]\S?-?\S?\S?\S?)-5` → `$1-spacious`
    7. **All numbers**: Search for `[mp]\S?-?\S?\S?\S?-\d` and you shouldn't have Bootstrap Spacing utilities anymore
  - Change all the utilities using more precise utilities
    1. **Mixing Bootstrap numbers**: Search for `[mp]\S?-?\S?\S?\S?-\d`, when you match multiple on one line, consider using one of our [scaled spacing utilities]({{< docsref "/utilities/spacing#notation-for-scaled-values" >}}) (for example, `.mb-1.mb-md-2` → `.mb-scaled-shortest`).
