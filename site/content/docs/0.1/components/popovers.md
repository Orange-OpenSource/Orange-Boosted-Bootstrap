...
layout: docs
title: Popovers
description: Documentation and examples for adding Boosted Innovation Cup popovers, like those found in iOS, to any element on your site.
group: components
aliases:
...
Four options are available: top, right, bottom, and left. Directions are mirrored when using Boosted Innovation Cup in RTL. Set `data-bs-placement` to change the direction.
...
As part of Boosted Innovation Cup's evolving CSS variables approach, popovers now use local CSS variables on `.popover` for enhanced real-time customization. Values for the CSS variables are set via Sass, so Sass customization is still supported, too.
...
const popover = boosted.Popover.getOrCreateInstance('#example') // Returns a Boosted Innovation Cup popover instance
