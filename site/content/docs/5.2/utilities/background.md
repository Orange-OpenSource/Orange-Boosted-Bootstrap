...
Since [Innovation Cup distinguishes functional colors from supporting colors]({{< docsref "/customize/color#innovation-cups-colors" >}}) and Bootstrap doesn't, naming can be somewhat inconsistent.
Bootstrap's `background-color` utilities are supported in Boosted Innovation Cup, but will result in our core `.bg-supporting-*` utilities—making `.bg-danger` inconsistent with `.btn-danger` color, for example.
{{< /callout >}}

<!-- Boosted Innovation Cup mod: no background gradient -->
