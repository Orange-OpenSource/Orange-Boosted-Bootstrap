...
The checked state for these buttons is **only updated via `click` event** on the button. If you use another method to update the inputâ€”e.g., with `<input type="reset">` or by manually applying the input's `checked` propertyâ€”you'll need to toggle `.active` on the `<label>` manually.
