// Playground for Static list item
...
// asset table, the small-item warning and the `State` select — it is not
// repeated here. This file only comments what a list item changes.
//
...
// blocked on the kit.
//
// A LIST ITEM IS `<li class="item">` INSIDE `<ul class="item-list">`, NOT A
// CLASS ON THE SAME `<div>` A CARD ITEM USES. That is the whole reason this is
// a separate file rather than a "type" select on Static card item: the DOM
// shape differs, not just a modifier class. The `<ul>` wraps a single `<li>`
// here — enough to show the list scaffolding (`.item-list` sets
// `box-sizing`, removes list markers, zeroes the background and the border
// radius) without the added complexity of a second, frozen item per
// conventions.md §9 ("Radio button item — 2 options" is the precedent for
// freezing rather than adding a count control that teaches nothing new).
//
// `Max width` is `.component-max-width` on the `<ul>`, exactly as the docs'
// own list example shows — not on the `<li>`.
//
// `Outlined` and `Rounded corners` ARE NOT OFFERED HERE.
// - `Outlined`: the docs' own callout says "since list items are sticked,
//   this variant should only be used on card items" — a documented
//   restriction on the whole component, not a combination to warn about
//   within one story (conventions.md §7's gating principle, applied to a
//   variant rather than a wrapper).
// - `Rounded corners`: the stylesheet sets `.item-list` after
//   `.use-rounded-corner-items` specifically to override it back to
//   `$ouds-list-item-border-radius-none` — the docs say so too ("Items in a
//   list won't have rounded corners even using this class"). A control that
//   changes nothing must not exist (conventions.md §1).
...
const renderStaticListItem = ({ overline, label, boldLabel, extraLabel, description, leadingAsset, trailingAsset, helperText, state, background, noDivider, topAlignment, smallSize, maxWidth }, icons = inlineIcons, preview = true) => {
...
export default {
  title: 'Playground/Static list item',
  argTypes: {
    overline: {
      name: 'Overline',
      control: 'text',
    ...
    leadingAsset: {
      ...
      description: 'One control for asset type and size together: the stylesheet only ever compounds the pairs the design system draws.',
    },
    trailingAsset: {
      name: 'Trailing asset',
      control: 'select',
    ...
    maxWidth: {
      name: 'Max width',
      control: 'boolean',
      description: '`.component-max-width` on the `<ul class="item-list">`, exactly as the documentation example shows.',
    }
  }
}

export const PlaygroundStaticListItem = {
  parameters: {
    docs: {
      codePanel: true,
      source: {
        transform: (_src, context) => renderStaticListItem(context.args, spriteIcons, false),
      },
    },
  },
  render: (args) => renderStaticListItem(args),
  args: {
    overline: 'Overline',
    label: 'Label',
    boldLabel: false,
