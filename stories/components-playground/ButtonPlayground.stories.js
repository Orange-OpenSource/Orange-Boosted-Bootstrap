...
// Le tracé ci-dessus sert au canvas : il s'affiche sans dépendre du sprite
// hébergé. Le panneau Code, lui, montre la référence au sprite — c'est ce que
// l'on écrit dans une vraie page, et ça évite 450 caractères de `d="M851…"`
// dans un snippet destiné à être copié.
...
// `iconOverride` n'est passé que par le panneau Code, et seulement tant que
// l'utilisateur n'a pas saisi son propre tracé : dans ce cas on affiche le sien,
// sinon le snippet mentirait sur ce qu'il rend.
const renderButton = ({ label, variant, layout, state, icon, rounded }, iconOverride) => {
  ...
  const iconMarkup = iconOverride || renderIcon(icon || defaultIcon, true)
