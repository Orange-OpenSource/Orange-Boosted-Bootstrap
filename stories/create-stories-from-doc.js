  ...
  for (const file of files) {
    ...
    try {
      ...
      for (const example of e) {
        ...
        example[0] += `\n<script type="text/javascript">\n  var root = document.getElementById('storybook-root') || document.getElementById('story--components-${file[0].toLowerCase()}--${convertToKebabCase(file[0])}-${index}-inner'); root.className = "${Object.values(example[1]).join(' ')} m-0 p-0 border-0"\n</script>` // Add example classes and remove the previous ones
