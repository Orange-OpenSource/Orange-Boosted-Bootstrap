...
Use the `margin` and `padding` [spacing utilities]([[docsref:/utilities/spacing]]) to control how elements and components are spaced and sized. OUDS Web includes an eleven-level scale for spacing utilities, based on a <code>{getTokenValue('$core-ouds-dimension-base')}</code> value default <code>{allTokens.find((token) => token.name === '$core-ouds-dimension-base').name}</code> variable. Choose values for all viewports (e.g., `.me-large` for <code>margin-right: {getTokenValue('$ouds-space-fixed-large')}</code> in LTR), or pick responsive variants to target specific viewports (e.g., `.md:me-large` for <code>margin-right: {getTokenValue('$ouds-space-fixed-large')}</code> —in LTR— starting at the `md` breakpoint).
