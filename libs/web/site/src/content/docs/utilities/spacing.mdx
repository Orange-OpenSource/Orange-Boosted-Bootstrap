...
export const fixedSpacingLevel = [
  "none",
  "3xsmall",
  "2xsmall",
  "xsmall",
...
export const scaledSpacingLevel = [
  "none",
  "3xsmall",
  "2xsmall",
  "xsmall",
...
The classes are named using the format `{property}{sides}-{size}` for `2xs` and `{breakpoint}:{property}{sides}-{size}` for `xs`, `sm`, `md`, `lg`, `xl`, `2xl`, and `3xl`.
...
<ul>
  {
    fixedSpacingLevel.map((level) => (
      <li><code>{level}</code> - for classes that set
        the <code>margin</code> or <code>padding</code> to <code>{getTokenValue(`$ouds-space-fixed-${level}`)}</code>
      </li>
    ))
  }
  <li>`auto` - for classes that set the `margin` to `auto`</li>
  <li>`gridgap` - for classes that set the `margin` to `var(--bs-grid-gap)` [see below](#grid-measures)</li>
  <li>`gridmargin` - for classes that set the `margin` to `var(--bs-grid-margin)` [see below](#grid-measures)</li>
</ul>
...
- `2` - (by default) for classes that set the `margin` or `padding` to `.75rem`
- `3` - (by default) for classes that set the `margin` or `padding` to `1.5rem`
- `4` - (by default) for classes that set the `margin` or `padding` to `2.5rem`
- `5` - (by default) for classes that set the `margin` or `padding` to `3.5rem`
...
<div class="table-responsive">
  <table class="table">
    ...
    <tr>
      <th>Size name</th>
      <th>Size for mobile (`2xs` to `sm`)</th>
      <th>Size for tablet (`md` to `lg`)</th>
      <th>Size for desktop (`xl` and upper)</th>
    </tr>
    </thead>
    <tbody>
    {
      scaledSpacingLevel.map((level) => (
        <tr>
          <td>
            <code>{level}</code>
          </td>
          {
            screenSize.map((size) => (
              <td>
                <code>{getTokenValue(`$ouds-space-scaled-${level}-${size}`)}</code>
              </td>
            ))
          }
        </tr>
...
<Example class="bd-example-spacings" code={`<div class="mx-scaled-large">Large scaled x margin</div>
  ...
  <div class="mx-large md:mx-xlarge xl:mx-2xlarge">Large scaled x margin (manually)</div>`} />
...
<Callout type="info">
 The [negative margins for grid measures](#grid-measures) are always available, irrespective of `$enable-negative-margins` value.
</Callout>
...
OUDS web provides two specific sizes based on grid measures: `gridgap` and `gridmargin` which use the custom properties `--bs-grid-gap` and `--bs-grid-margin`, respectively. These custom properties are responsive so the values will depend on the current viewport breakpoint. These utilities are available as `margin`, `padding` or `gap` and can be applied on a specific breakpoint, they use the [same notation as fixed values](#notation-for-fixed-values).

<Example class="bd-example-spacings" code={`<div class="px-gridgap">Gridgap padding</div>
  <div class="xl:px-gridgap">Grid-gap padding for xl viewports</div>
  <div class="mt-gridmargin">Grid-margin margin viewports</div>`} />

### Edge to edge containers

<Callout type="info">
  The negative margins for grid measures are always available, unlike [the other negative margins](#negative-margin).
</Callout>
...
<div class="mx-ngridmargin lg:mx-none py-small lg:px-small px-gridmargin mb-small" style="background-color: rgba(var(--bd-primary-light-rgb), 0.15);">Edge to edge container on smaller breakpoints</div>
