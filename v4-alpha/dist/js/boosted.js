...
}if(index < 0){index = $items.length - 1;}if(index === $items.length){index = 0;}$items.eq(index).focus();e.preventDefault();e.stopPropagation();}},{key:'_getTargetFromElement',value:function _getTargetFromElement(element){var selector=Util.getSelectorFromElement(element);return selector?$(selector)[0]:null;}},{key:'_jQueryInterface',value:function _jQueryInterface(config){return this.each(function(){var $this=$(this);var data=$this.data(DATA_KEY);var _config=$.extend({},Default,$this.data(),typeof config === 'object' && config);if(!data && _config.toggle && /show|hide/.test(config)){_config.toggle = false;}if(!data){data = new Collapse(this,_config);$this.data(DATA_KEY,data);}if(typeof config === 'string'){if(data[config] === undefined){throw new Error('No method named "' + config + '"');}data[config]();}});}},{key:'VERSION',get:function get(){return VERSION;}},{key:'Default',get:function get(){return Default;}}]);return Collapse;})();var uniqueId=function uniqueId(prefix){return (prefix || 'ui-id') + '-' + Math.floor(Math.random() * 1000 + 1);};var $colltabs=$('[data-toggle="collapse"]:not(.navbar-toggle)');$colltabs.attr({role:'tab','aria-selected':'false','aria-expanded':'false'});$colltabs.each(function(){var collid=$(this).attr('id') || uniqueId('ui-collapse'),collpanel=$(this).attr('data-target')?$($(this).attr('data-target')):$($(this).attr('href')),collparent=parent && $(parent),colltab=$(this),heading='',parent=colltab.attr('data-parent');colltab.attr('id',collid);if(collparent){$(collparent).find('div:not(.collapse,.panel-body), h4').attr('role','presentation');collparent.attr({role:'tablist','aria-multiselectable':'true'});heading = collpanel.parent().children().first(); // On sélectionne le heading (panel-heading)
...
value:function _activate(element,container,callback){var $active=$(container).find('> .active');$active.find('[data-toggle=tab], [data-toggle=pill]').attr({tabIndex:'-1','aria-selected':false});$active.filter('.tab-pane').attr({'aria-hidden':true,tabIndex:'-1'});var active=$(container).find(Selector.ACTIVE_CHILD)[0];var isTransitioning=callback && Util.supportsTransitionEnd() && (active && $(active).hasClass(ClassName.FADE) || Boolean($(container).find(Selector.FADE_CHILD)[0]));var complete=$.proxy(this._transitionComplete,this,element,active,isTransitioning,callback);if(active && isTransitioning){$(active).one(Util.TRANSITION_END,complete).emulateTransitionEnd(TRANSITION_DURATION);}else {complete();}if(active){$(active).removeClass(ClassName.IN);}if(element.tagName === 'A'){var elemID='#' + element.id;$(elemID).attr({tabIndex:'0','aria-selected':true}).focus();}$(element).filter('.tab-pane.active').attr({'aria-hidden':false,tabIndex:'0'});}},{key:'_transitionComplete',value:function _transitionComplete(element,active,isTransitioning,callback){if(active){$(active).removeClass(ClassName.ACTIVE);var dropdownChild=$(active).find(Selector.DROPDOWN_ACTIVE_CHILD)[0];if(dropdownChild){$(dropdownChild).removeClass(ClassName.ACTIVE);}active.setAttribute('aria-expanded',false);}$(element).addClass(ClassName.ACTIVE);element.setAttribute('aria-expanded',true);if(isTransitioning){Util.reflow(element);$(element).addClass(ClassName.IN);}else {$(element).removeClass(ClassName.FADE);}if(element.parentNode && $(element.parentNode).hasClass(ClassName.DROPDOWN_MENU)){var dropdownElement=$(element).closest(Selector.DROPDOWN)[0];if(dropdownElement){$(dropdownElement).find(Selector.DROPDOWN_TOGGLE).addClass(ClassName.ACTIVE);}element.setAttribute('aria-expanded',true);}if(callback){callback();}} // static
}],[{key:'_keydown',value:function _keydown(e){var $this=$(this);var $items=undefined;var $ul=$this.closest('ul[role=tablist] ');var index=undefined;var k=e.which || e.keyCode;$this = $(this);if(!/(37|38|39|40)/.test(k)){return;}$items = $ul.find('[role=tab]:visible');index = $items.index($items.filter(':focus'));if(k === 38 || k === 37){index--;} // up & left
if(k === 39 || k === 40){index++;} // down & right
if(index < 0){index = $items.length - 1;}if(index === $items.length){index = 0;}var nextTab=$items.eq(index);if(nextTab.attr('role') === 'tab'){nextTab.tab('show').focus();}e.preventDefault();e.stopPropagation();}},{key:'_jQueryInterface',value:function _jQueryInterface(config){return this.each(function(){var $this=$(this);var data=$this.data(DATA_KEY);if(!data){data = data = new Tab(this);$this.data(DATA_KEY,data);}if(typeof config === 'string'){if(data[config] === undefined){throw new Error('No method named "' + config + '"');}data[config]();}});}},{key:'VERSION',get:function get(){return VERSION;}}]);return Tab;})();$(document).on(Event.CLICK_DATA_API,Selector.DATA_TOGGLE,function(event){event.preventDefault();Tab._jQueryInterface.call($(this),'show');});$(document).on('keydown.tab.data-api','[data-toggle="tab"], [data-toggle="pill"]',function(event){if(!/(38|40|39|37)/.test(event.which)){return;}event.preventDefault();Tab._keydown.call($(this),event);}); /**
   * ------------------------------------------------------------------------
   * jQuery
   * ------------------------------------------------------------------------
   */ // ajout de l'accesibilité
// ===============================
var uniqueId=function uniqueId(prefix){return (prefix || 'ui-id') + '-' + Math.floor(Math.random() * 1000 + 1);};var $tablist=$('.nav-tabs, .nav-pills');var $lis=$tablist.children('li');var $tabs=$tablist.find('[data-toggle="tab"], [data-toggle="pill"]');$tablist.attr('role','tablist');$lis.attr('role','presentation');$tabs.attr('role','tab');$tabs.each(function(){var tabpanel=$($(this).attr('href'));var tab=$(this);var tabid=tab.attr('id') || uniqueId('ui-tab');tab.attr('id',tabid);if(tab.hasClass('active')){tab.attr({tabIndex:'0','aria-selected':'true','aria-controls':tab.attr('href').substr(1)});tabpanel.attr({role:'tabpanel',tabIndex:'0','aria-hidden':'false','aria-labelledby':tabid});}else {tab.attr({tabIndex:'-1','aria-selected':'false','aria-controls':tab.attr('href').substr(1)});tabpanel.attr({role:'tabpanel',tabIndex:'-1','aria-hidden':'true','aria-labelledby':tabid});}});$.fn[NAME] = Tab._jQueryInterface;$.fn[NAME].Constructor = Tab;$.fn[NAME].noConflict = function(){$.fn[NAME] = JQUERY_NO_CONFLICT;return Tab._jQueryInterface;};return Tab;})(jQuery); /* global Tether */ /**
 * --------------------------------------------------------------------------
 ...
 * Licensed under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)
 * --------------------------------------------------------------------------
 */var Megamenu=(function($){'use strict';var megamenuItems='.mega-menu ul.navbar-nav > li > a';function uuid(){var id=Math.random().toString(16) + '000000000';return 'ui-' + id.substr(2,8);} // arrow keys management
...
$('.mega-menu .dropdown-toggle').each(function(){var dropdownToggleId=uuid();var dropdownMenu=$(this).next('ul.dropdown-menu');var dropdownMenuId=uuid();$(this).attr('id',dropdownToggleId);$(dropdownMenu).attr('id',dropdownMenuId);$(this).attr('aria-owns',dropdownMenuId);$(this).attr('aria-controls',dropdownMenuId);$(dropdownMenu).attr('aria-labelledby',dropdownToggleId);});$('.mega-menu .megamenu-dropdown-toggle').on('click',function(){$('.mega-menu .megamenu-dropdown-toggle').each(function(){$(this).removeClass('active');});$(this).addClass('active');}); // extending to level 2 in 320
$('.mega-menu h3').on('click',function(){$('.megamenuclosetop').parent().find('.megamenuclosetop').addClass('active');$('.megamenuclosetop').parent().parent().parent().parent().find('.container .navbar-toggleable-xs').addClass('active');$(this).addClass('disabled');}); // extending to level 1 in 320
$('.nav-item .megamenu-dropdown-toggle').on('click',function(){$('.closing-bare').css({height:'0'});$('.open').removeClass('open'); // standard
$(this).parent().addClass('open');$(this).attr('aria-expanded','true'); // 320
if(document.body.clientWidth < 768){$(this).parent().parent().addClass('menu-level1');var _outerHeight=$(this).parent().find('ul[role=menu]').outerHeight();$('.navbar-toggleable-xs').css({height:_outerHeight});}else {$('.navbar-toggleable-xs').css({height:'inherit'});$('.closing-bare').animate({height:'40px'},200);}}); // extending to level 2 in 320
$('.nav-item ul li div h3').on('click',function(){$(this).parent().find('ul').addClass('open');$(this).parent().find('ul').prepend('<li class="back_menu1" ><h4 class="back-menu1title">' + this.innerHTML + '</h4></li>');$('.back_menu1').on('click',backToMenu1);if(document.body.clientWidth < 768){var _outerHeight2=$(this).parent().find('ul').outerHeight();$('.navbar-toggleable-xs').css({height:_outerHeight2});$('.menu-level1').addClass('menu-level2');}}); // back to level 0 in 320
$('.nav-item .row h2').on('click',function(){$('.menu-level1').removeClass('menu-level1');$('.navbar-toggleable-xs').css({height:'inherit'});}); // back to level 1 in 320
function backToMenu1(){$('.menu-level1').removeClass('menu-level2');$('.back_menu1').remove();$('.row .open').removeClass('open');if(document.body.clientWidth < 768){var _outerHeight3=$('.menu-level1 > .nav-item.open ul ').outerHeight();$('.navbar-toggleable-xs').css({height:_outerHeight3});}} //  openning mega menu in 320
$('.megamenubacktop').on('click',function(){$('.menu-level1').removeClass('menu-level1').removeClass('menu-level2');$('.navbar-toggleable-xs').css({height:'inherit'});$('.topmenucontent .icon-search').removeClass('color-orange');$('.megamenubacktop').parent().find('.megamenuclosetop').addClass('active');$('.megamenuclosetop').parent().parent().parent().parent().find('.container .navbar-toggleable-xs').addClass('active');$('.megamenuclosetop').parent().parent().parent().parent().find('.navbar-langbar').addClass('active');$('.megamenuclosetop').parent().parent().parent().parent().find('.menu-footer-320').addClass('active');$('.navbar-smallsearchbar').removeClass('active');$(this).addClass('disabled');}); // close mega menu in 320
$('.megamenuclosetop , .closing-bare-button a').on('click',function(){$('.open').removeClass('open');$('.megamenuclosetop').parent().find('.megamenubacktop').removeClass('disabled');$('.megamenuclosetop').parent().parent().parent().parent().find('.container .navbar-toggleable-xs').removeClass('active');$('.megamenuclosetop').parent().parent().parent().parent().find('.navbar-langbar').removeClass('active');$('.megamenuclosetop').parent().parent().parent().parent().find('.menu-footer-320').removeClass('active');$(this).removeClass('active');}); // search bar
$('.megamenusearchtop').on('click',function(){$('.megamenuclosetop').trigger('click');$('.topmenucontent .icon-search').addClass('color-orange');$('.navbar-smallsearchbar').addClass('active');}); // clear search
$('.megamenusearchinputclose').on('click',function(){$(this).parent().find('input').val('');$(this).removeClass('visible');});$('.megamenusearchinputtext').on('keyup',function(){if($(this).val() !== ''){$(this).parent().find('.megamenusearchinputclose').addClass('visible');}else {$(this).parent().find('.megamenusearchinputclose').removeClass('visible');}});})(jQuery); /**
 * --------------------------------------------------------------------------
