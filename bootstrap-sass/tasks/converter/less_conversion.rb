  ...
  module LessConversion
    ...
    def process_stylesheet_assets
      ...
      files.each do |name, file|
        ...
        file = replace_all file, /\.\.\/\.\.\/bower_components\/bootstrap\/less\//, '' if file =~ /bower_components/
        #file = replace_all file, /@import "variables";/, '' if file =~ /@import "variables";/
        file = replace_all file, /@import "variables"/, '@import "variables-orange2015"' if file =~ /@import "variables"/
        ...
        case name
          ...
          when 'mixins/step_bar.less'
            # replace the var name by the full expression
            file = replace_all file, /23\$color/, '23#{$color}'
            # return the value instead of defining a var
            file = replace_all file, /\$arrow-svg:/, '@return'
            # replace the mixin by a function
            file = replace_all file, /@mixin arrow-svg/, '@function arrow-svg'                        
            # replace include by var
            file = replace_all file, /@include/, '$arrow-svg:'            
          when 'mixins/grid-framework.less'
            file = convert_grid_mixins file
          when 'mixins/nav-vertical-align.less'
            file = "@mixin navbar-vertical-align($element-height){ 
                      margin-bottom: 0;                          
          ...
          when 'variables.less'
            ...
            SCSS
          when 'variables-orange2015.less'
            file = insert_default_vars(file)
            file = ['$bootstrap-sass-asset-helper: false !default;', file].join("\n")
            file = replace_all file, %r{(\$icon-font-path): \s*"(.*)" (!default);}, "\n" + unindent(<<-SCSS, 14)
              // [converter] If $bootstrap-sass-asset-helper if used, provide path relative to the assets load path.
              // [converter] This is because some asset helpers, such as Sprockets, do not work with file-relative paths.
              \\1: if($bootstrap-sass-asset-helper, "bootstrap/", "\\2") \\3;
            SCSS
      ...
      main_from = "#{save_to}/_bootstrap-orange2015.scss"
      main_to   = File.expand_path("#{save_to}/../bootstrap-orange2015.scss")
      save_file main_to, File.read(main_from).gsub(/ "/, ' "bootstrap/')
      File.delete(main_from)
      
      main_from = "#{save_to}/_boosted2015.scss"
      main_to   = File.expand_path("#{save_to}/../boosted2015.scss")
      save_file main_to, File.read(main_from).gsub(/ "/, ' "bootstrap/')
      File.delete(main_from)
      
      main_from = "#{save_to}/_orangeIcons.scss"
      main_to   = File.expand_path("#{save_to}/../orangeIcons.scss")
      save_file main_to, File.read(main_from).gsub(/variables-orange2015/, 'bootstrap/variables-orange2015')
      File.delete(main_from)
      
      main_from = "#{save_to}/_orangeHelvetica.scss"
      main_to   = File.expand_path("#{save_to}/../orangeHelvetica.scss")
      save_file main_to, File.read(main_from).gsub(/variables-orange2015/, 'bootstrap/variables-orange2015')
      File.delete(main_from)
      ...
      save_file 'templates/project/_bootstrap-variables.sass',
                "// Override Bootstrap variables here (defaults from bootstrap-sass v#{Bootstrap::VERSION}):\n\n" +
                    File.read("#{save_to}/_variables-orange2015.scss").lines[1..-1].join.gsub(/^(?=\$)/, '// ').gsub(/ !default;/, '')
    end

    def bootstrap_less_files
      @bootstrap_less_files = Dir.glob('tmp/boosted/less/*.less').map{ |f| File.basename f }.concat( Dir.glob('tmp/boosted/less/mixins/*.less').map{ |f| 'mixins/' + (File.basename f) })      
    end
...
SASS
    ...
    def replace_file_imports(less, target_path = '') 
      less = replace_all less, /\.\.\/\.\.\/bower_components\/bootstrap\/less\//, '' if less =~ /bower_components/
